...
struct Timing {
  uint32_t frame{};
  uint32_t phase{}, phase_frame{}, backgrounds{},host_flag{};
...
struct Wuc {
  ...
  bool toggle{}, reverse{},flag_toggle{};
  HWND window{};
  Wuc(HWND window,Renderer& renderer,const std::wstring& mode){
    ...
    flag_toggle=mode==L"flag-toggle"||mode==L"flag-toggle-reverse";
    toggle=mode==L"toggle"||mode==L"toggle-reverse"||flag_toggle;
    reverse=mode==L"toggle-reverse"||mode==L"flag-toggle-reverse";
    if(mode==L"sparse"||mode==L"elided"||toggle){
  ...
  void submit(Renderer& renderer,Timing& t){
    ...
    t.mutation=qpc();
    if(toggle){
      // Keep one scene and all four backdrop nodes alive. Only change their
      // opacity at a phase boundary, inside the same surface update.
      t.phase=(t.frame-1)/240;t.phase_frame=(t.frame-1)%240;
      const bool enabled=(t.phase%2==0)!=reverse;
      t.backgrounds=(!flag_toggle&&enabled)?4u:0u;
      t.host_flag=flag_toggle?unsigned(enabled):1u;
      if(t.phase_frame==0){
        for(auto& [key,node]:scene->nodes)
          if(node.background)node.background.Opacity(t.backgrounds?1.0f:0.0f);
        if(flag_toggle){BOOL flag=BOOL(t.host_flag);check_hresult(DwmSetWindowAttribute(window,DWMWA_USE_HOSTBACKDROPBRUSH,&flag,sizeof(flag)));}
      }
    }
...
int run(const std::wstring& mode,UINT duration,FILE* log){
  ...
  const bool toggle=mode==L"toggle"||mode==L"toggle-reverse"||mode==L"flag-toggle"||mode==L"flag-toggle-reverse";
  const bool host_flag=mode==L"hostflag"||mode==L"sparse"||mode==L"elided"||toggle;
  ...
  std::fprintf(log,"probe-scene host_flag=%u sparse=%u backgrounds=%u diagnostic_elision=%u\n",unsigned(host_flag),unsigned(mode==L"sparse"||mode==L"elided"||toggle),(mode==L"sparse"||toggle)?4u:0u,unsigned(mode==L"elided"));
  if(toggle)std::fprintf(log,"probe-toggle phase_frames=240 warmup_frames=30 nodes_retained=1 reverse=%u flag_toggle=%u\n",unsigned(mode==L"toggle-reverse"||mode==L"flag-toggle-reverse"),unsigned(mode==L"flag-toggle"||mode==L"flag-toggle-reverse"));
  ...
  for(const auto& t:timings)std::fprintf(log,"probe-frame frame=%u phase=%u phase_frame=%u backgrounds=%u host_flag=%u render_qpc=%lld drawn_qpc=%lld begin_qpc=%lld copied_qpc=%lld ended_qpc=%lld mutation_qpc=%lld committed_qpc=%lld\n",t.frame,t.phase,t.phase_frame,t.backgrounds,t.host_flag,t.render,t.drawn,t.begin,t.copied,t.ended,t.mutation,t.committed);
...
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR,int){
  ...
  const bool valid=(mode==L"winrt"||mode==L"dcomp"||mode==L"hostflag"||mode==L"sparse"||mode==L"elided"||mode==L"toggle"||mode==L"toggle-reverse"||mode==L"flag-toggle"||mode==L"flag-toggle-reverse")&&end&&!*end&&duration>=1000&&duration<=60000;LocalFree(args);if(!valid)return 2;
