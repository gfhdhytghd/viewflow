...
impl TouchpadState {
    pub(crate) fn apply<E>(
        ...
        if old.iter().any(|c| !current.iter().any(|n| n.id == c.id)) {
            ...
            let lifted: Vec<_> = old
                .iter()
                .map(|c| {
                    current
                        .iter()
                        .find(|n| n.id == c.id)
                        .map_or((*c, false), |n| (*n, true))
                })
                .collect();
            inject(&lifted)?;
            let mut remaining = TouchpadFrame {
                width: frame.width,
                height: frame.height,
            ...
            for c in old
                .iter()
                .filter_map(|c| current.iter().find(|n| n.id == c.id))
            {
...
mod tests {
    ...
    fn frame(ids: &[u32]) -> TouchpadFrame {
        let mut f = TouchpadFrame {
            width: 10000,
            height: 6000,
            count: ids.len() as u8,
            ..TouchpadFrame::default()
        };
        for (c, id) in f.contacts.iter_mut().zip(ids) {
            *c = TouchpadContact {
                id: *id,
                x: 1000,
                y: 2000,
    ...
    fn five_finger_replacement_lifts_old_ids_before_new_ones() {
        ...
        state
            .apply(frame(&[1, 2, 3, 4, 5]), |_| Ok::<_, ()>(()))
            .unwrap();
        let mut calls = vec![];
        state
            .apply(frame(&[6, 7, 8, 9, 10]), |events| {
    ...
    fn failed_lifts_remain_tracked_and_successful_lifts_are_not_replayed() {
        ...
        state
            .apply(frame(&[]), |events| {
                released.extend_from_slice(events);
                Ok::<_, ()>(())
            })
            .unwrap();
