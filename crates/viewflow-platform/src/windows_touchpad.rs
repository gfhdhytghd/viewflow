...
use windows_sys::Win32::{
    ...
    UI::{
        Controls::{
            DestroySyntheticPointerDevice, HSYNTHETICPOINTERDEVICE, POINTER_FEEDBACK_NONE,
            POINTER_TYPE_INFO,
        },
        Input::Pointer::{
...
impl Default for WindowsTouchpad {
    fn default() -> Self {
        Self {
            ...
            state: crate::touchpad_state::TouchpadState::default(),
            diagnostic_frames: 0,
            diagnostic_failures: 0,
            diagnostic_count: u8::MAX,
        }
    }
}
...
impl WindowsTouchpad {
    fn create(&mut self, frame: TouchpadFrame) -> Result<(), WindowsInputError> {
        ...
        let address =
            unsafe { GetProcAddress(module, c"CreateSyntheticPointerDevice2".as_ptr().cast()) }
        ...
        self.state.previous = TouchpadFrame {
            width: frame.width,
            height: frame.height,
    ...
    fn inject(
        device: HSYNTHETICPOINTERDEVICE,
        contacts: impl Iterator<Item = (TouchpadContact, bool)>,
    ) -> Result<(), WindowsInputError> {
        let mut inputs = Vec::with_capacity(5);
        for (c, touching) in contacts {
            let mut info = POINTER_TYPE_INFO {
                r#type: PT_TOUCHPAD,
                ..POINTER_TYPE_INFO::default()
            };
        ...
        if !inputs.is_empty()
            && unsafe { InjectSyntheticPointerInput(device, inputs.as_ptr(), inputs.len() as u32) }
                == 0
        {
            eprintln!(
                "Viewflow touchpad injection failed: {}",
    ...
    pub(crate) fn apply(&mut self, frame: TouchpadFrame) -> Result<(), WindowsInputError> {
        ...
        if self.diagnostic_count != frame.count
            || (result.is_err() && self.diagnostic_failures.is_power_of_two())
        {
            use std::io::Write;
            let message = format!(
                "touchpad frames={} count={} size={}x{} device={} failures={} result={result:?} native_error={native_error}",
    ...
    fn apply_frame(&mut self, frame: TouchpadFrame) -> Result<(), WindowsInputError> {
        ...
        if !self.device.is_null()
            && (frame.width, frame.height)
                != (self.state.previous.width, self.state.previous.height)
        {
            self.release_all()?;
            self.destroy();
        }
        ...
        self.state.apply(frame, |contacts| {
            Self::inject(device, contacts.iter().copied())
        })
    }
    ...
    pub(crate) fn release_all(&mut self) -> Result<(), WindowsInputError> {
        ...
        let empty = TouchpadFrame {
            count: 0,
            ..self.state.previous
        };
        let device = self.device;
        self.state.apply(empty, |contacts| {
            Self::inject(device, contacts.iter().copied())
        })
    }
