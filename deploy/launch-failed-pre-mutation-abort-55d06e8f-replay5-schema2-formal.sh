...
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-pre-mutation-abort-55d06e8f-replay5-schema2-manifest.json
readonly MANIFEST_SHA256=9a0cb489a973c1d4d19c2c5a168d4ea3942a679c6fe329f99d35f6a9ccabf43a
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/55d06e8f96aa4adc9010e53612979374/failed-pre-mutation-abort-replay5-schema2-execution-approval.json
...
expected_manifest_keys={"active_marker_gate","adopted_outputs","approval","argv","execution_authorized","fresh_outputs","immutable_terminal_inputs","initial_approval","installed_v13_inputs","marker_candidate","operation_id","prior_approval","required_absent","reviewed_code","schema_version","state","superseded_replay3","superseded_replay4","threat_boundary"}
if not (set(manifest)==expected_manifest_keys and manifest.get("schema_version")==2 and manifest.get("state")=="viewflow-failed-pre-mutation-abort-replay5-schema2-command-manifest" and manifest.get("execution_authorized") is False and manifest.get("operation_id")==op and manifest.get("threat_boundary")=="cooperating-crash-and-non-owner"):
    raise SystemExit(67)
...
if set(outputs)!=expected_output_keys or len(set(outputs.values()))!=6 or any(not isinstance(path,str) or not path.startswith("/home/wilf/.local/state/viewflow/deployments/"+op+"/") or ".replay5." not in path for path in outputs.values()):
    raise SystemExit(67)
...
def validate_absent(include_approval):
    absent=[*required_absent,*outputs.values(),superseded["approval_path"],*superseded_outputs,superseded4["approval_path"],*superseded4_outputs]
    if include_approval:
