...
readonly LAUNCHER=/home/wilf/data/viewflow/deploy/launch-failed-v13-abort-481366c6-replay17-formal.sh
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-v13-abort-481366c6-replay17-manifest.json
readonly MANIFEST_SHA256=056e766b4d29235aeab05265bdaa2be825705fe1c5fcc3bf1d36b8d6d5f24959
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-replay17-execution-approval.json
...
op,manifest_path,manifest_sha,approval_path,approval_sha,launcher_sha,mode=sys.argv[1:]
...
if not (manifest.get("schema_version")==1 and manifest.get("state")=="viewflow-failed-v13-abort-replay17-command-manifest" and manifest.get("execution_authorized") is False and manifest.get("operation_id")==op and manifest.get("recovery_boundary")=="post-vfdqa-committed-pre-terminal-transition"):
    raise SystemExit(67)
...
if policy!={"abort_command_forbidden":True,"pinned_query_required":True,"marker_required_absent":True,"abort_claim_required_absent":True,"release_claim_required_absent":True,"runtime_marker_required_absent":True}:
    raise SystemExit(67)
expected_approval_keys={"abort_authorization_sha256","approved","approved_at_utc","checker_sha256","coordinator_sha256","durable_abort_receipt_sha256","launcher_sha256","local_abort_receipt_sha256","negative_test_sha256","operation_id","prior_approval_sha256","prior_launcher_sha256","prior_manifest_sha256","publication_method","replay_manifest_sha256","schema_version","semantic_test_sha256","slot_cleanup_receipt_sha256","state","terminal_state_sha256","transient_adoption_test_sha256"}
if not (set(approval)==expected_approval_keys and approval.get("schema_version")==17 and approval.get("state")=="viewflow-failed-v13-abort-replay17-execution-approved" and approval.get("approved") is True and approval.get("operation_id")==op and approval.get("publication_method")=="create-once-no-replace-and-parent-fsync"):
    raise SystemExit(67)
...
approval_expected={
    ...
    "prior_approval_sha256":prior["approval_sha256"],"prior_manifest_sha256":prior["manifest_sha256"],"prior_launcher_sha256":prior["launcher_sha256"],"terminal_state_sha256":gates["terminal_state_sha256"],
...
if any(approval.get(key)!=value for key,value in approval_expected.items()):
    raise SystemExit(67)
prior_manifest_source_fd,prior_manifest_stat,prior_manifest_bytes=open_exact(prior["manifest_path"],prior["manifest_sha256"],0o600)
prior_launcher_source_fd,prior_launcher_stat,prior_launcher_bytes=open_exact(prior["launcher_path"],prior["launcher_sha256"],0o700)
sealed={manifest_path:(seal_bytes("viewflow-replay17-manifest",manifest_bytes),manifest_source_fd,manifest_stat,manifest_bytes),approval_path:(seal_bytes("viewflow-replay17-approval",approval_bytes),approval_source_fd,approval_stat,approval_bytes),prior["manifest_path"]:(seal_bytes("viewflow-replay16-manifest",prior_manifest_bytes),prior_manifest_source_fd,prior_manifest_stat,prior_manifest_bytes),prior["launcher_path"]:(seal_bytes("viewflow-replay16-launcher",prior_launcher_bytes),prior_launcher_source_fd,prior_launcher_stat,prior_launcher_bytes)}
paths={prior["manifest_path"],prior["launcher_path"]}
...
if len(manifest["sealed_inputs"])!=29 or len(actual_sealed)!=29 or len(expected_sealed)!=29 or actual_sealed!=expected_sealed:
    raise SystemExit(67)
prior_manifest=json.loads(blob(prior["manifest_path"]).decode("utf-8"),object_pairs_hook=strict_pairs)
prior_manifest_keys={"abort_replay_policy","committed_abort","execution_authorized","existing_outputs","failed_deskflow_gate","fresh_transition","immutable_base_plan","immutable_gates","live_viewflow_gate","operation_id","prior_replay","recovery_boundary","reviewed_replay","schema_version","sealed_inputs","state"}
if set(prior_manifest)!=prior_manifest_keys or prior_manifest.get("schema_version")!=1 or prior_manifest.get("state")!="viewflow-failed-v13-abort-replay16-command-manifest" or prior_manifest.get("operation_id")!=op or prior_manifest.get("execution_authorized") is not False:
    raise SystemExit(67)
prior_approval=json.loads(blob(prior["approval_path"]).decode("utf-8"),object_pairs_hook=strict_pairs)
prior_keys={"abort_authorization_sha256","approved","approved_at_utc","checker_sha256","coordinator_sha256","durable_abort_receipt_sha256","launcher_sha256","local_abort_receipt_sha256","negative_test_sha256","operation_id","prior_approval_sha256","publication_method","replay_manifest_sha256","schema_version","semantic_test_sha256","slot_cleanup_receipt_sha256","state","terminal_state_sha256","transient_adoption_test_sha256"}
prior_reviewed=prior_manifest["reviewed_replay"]
prior_expected={"abort_authorization_sha256":committed["authorization_sha256"],"checker_sha256":prior_reviewed["checker_sha256"],"coordinator_sha256":prior_reviewed["coordinator_sha256"],"durable_abort_receipt_sha256":committed["durable_receipt_sha256"],"launcher_sha256":prior["launcher_sha256"],"local_abort_receipt_sha256":committed["local_receipt_sha256"],"negative_test_sha256":prior_reviewed["negative_test_sha256"],"operation_id":op,"prior_approval_sha256":"c34fe243b35d0564100965e604517a1a2a94e454d817e10edd0f108f4f4978dc","publication_method":"create-once-no-replace-and-parent-fsync","replay_manifest_sha256":prior["manifest_sha256"],"schema_version":16,"semantic_test_sha256":prior_reviewed["semantic_test_sha256"],"slot_cleanup_receipt_sha256":gates["slot_cleanup_receipt_sha256"],"state":"viewflow-failed-v13-abort-replay16-execution-approved","terminal_state_sha256":gates["terminal_state_sha256"],"transient_adoption_test_sha256":prior_reviewed["transient_adoption_test_sha256"]}
if set(prior_approval)!=prior_keys or prior_approval.get("approved") is not True or any(prior_approval.get(key)!=value for key,value in prior_expected.items()) or not isinstance(prior_approval.get("approved_at_utc"),str) or re.fullmatch(r"[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z",prior_approval["approved_at_utc"]) is None:
    raise SystemExit(67)
if not (prior_reviewed.get("launcher_path")==prior["launcher_path"] and prior_reviewed.get("approval_path")==prior["approval_path"] and prior_reviewed.get("coordinator_sha256")==prior_approval["coordinator_sha256"] and prior_reviewed.get("checker_sha256")==prior_approval["checker_sha256"] and prior_reviewed.get("semantic_test_sha256")==prior_approval["semantic_test_sha256"] and prior_reviewed.get("negative_test_sha256")==prior_approval["negative_test_sha256"] and prior_reviewed.get("transient_adoption_test_sha256")==prior_approval["transient_adoption_test_sha256"]):
    raise SystemExit(67)
...
def validate_failed_deskflow():
    ...
    if hashlib.sha256(exec_start).hexdigest()!=gate["exec_start_observation_sha256"]:
        raise SystemExit(68)
    output=subprocess.check_output(["/usr/bin/journalctl","--user","--quiet","--output","json","--all","_SYSTEMD_INVOCATION_ID="+gate["invocation_id"]],env=runtime_env)
    hits=[]
...
validate_absent();validate_viewflow();validate_failed_deskflow();reattest_sources()
if mode=="preflight":
    print("replay17 embedded sealed-FD preflight passed")
    raise SystemExit(0)
...
die() { printf 'failed-v13 replay17 launcher: %s\n' "$*" >&2; exit 1; }
