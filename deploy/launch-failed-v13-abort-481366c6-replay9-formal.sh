...
readonly LAUNCHER=/home/wilf/data/viewflow/deploy/launch-failed-v13-abort-481366c6-replay9-formal.sh
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-v13-abort-481366c6-replay9-manifest.json
readonly MANIFEST_SHA256=6317032250079496757034b336c313e899788d12351a5a784b3baa14fd120391
...
readonly COORDINATOR_SHA256=e8a52677b1ea1196a92f4b2329df1faa5db282e996ff35ad25c7791edc6c6ef9
readonly CHECKER=/home/wilf/data/viewflow/deploy/check-cross-host-coordinator.sh
readonly CHECKER_SHA256=4d852a14926b551740274a0596fdcf38b5c99c501e16b8b87070d1c16e85a960
...
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-replay9-execution-approval.json
...
if not (approval_object.get("schema_version")==9 and approval_object.get("state")=="viewflow-failed-v13-abort-replay9-execution-approved" and approval_object.get("approved") is True and approval_object.get("operation_id")==op):
    raise SystemExit(67)
if not (manifest_object.get("state")=="viewflow-failed-v13-abort-replay9-command-manifest" and manifest_object.get("execution_authorized") is False and manifest_object.get("operation_id")==op):
    raise SystemExit(67)
...
def validate_live_tuple():
    ...
    if [unit_value(unit,name) for name in ("ActiveState","SubState","Transient","KillMode")]!=["active","running","yes","control-group"]:
        raise SystemExit(68)
validate_live_tuple()
exec_fd=os.memfd_create("viewflow-replay9-coordinator",os.MFD_CLOEXEC|os.MFD_ALLOW_SEALING)
...
die() { printf 'failed-v13 replay9 launcher: %s\n' "$*" >&2; exit 1; }
...
jq -e --arg op "$OPERATION_ID" --arg manifest "$MANIFEST_SHA256" --arg launcher "$launcher_sha256" \
    ...
    .schema_version == 9 and .state == "viewflow-failed-v13-abort-replay9-execution-approved" and
...
' "/proc/$$/fd/7" >/dev/null || die 'replay9 approval receipt schema/binding differs'
...
jq -e --arg op "$OPERATION_ID" --arg plan "$PLAN_SHA256" --arg prior "$PRIOR_APPROVAL_SHA256" \
    ...
    .schema_version == 1 and .state == "viewflow-failed-v13-abort-replay9-command-manifest" and
    .execution_authorized == false and .operation_id == $op and
    .failure_class == "windows-task-xml-encoding-utf8-vs-utf16le-bom" and
...
' "/proc/$$/fd/5" >/dev/null || die 'replay9 manifest schema/binding differs'
