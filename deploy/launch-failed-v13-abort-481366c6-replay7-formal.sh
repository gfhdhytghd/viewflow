...
readonly LAUNCHER=/home/wilf/data/viewflow/deploy/launch-failed-v13-abort-481366c6-replay7-formal.sh
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-v13-abort-481366c6-replay7-manifest.json
readonly MANIFEST_SHA256=4bd2610b466dc60a11f5f501155754ac452748eed4a369b2de5dc4ad901fa1da
...
readonly COORDINATOR_SHA256=1f7f9fbc253fd775254fcfb8123d8483fc340ca5fa836841c9662a8f6bfe30cd
readonly CHECKER=/home/wilf/data/viewflow/deploy/check-cross-host-coordinator.sh
readonly CHECKER_SHA256=597f0a39b8ff84df5f1a88fdf72164735bfaab23d6aaaf0c33b9b813b7d43e8a
...
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-replay7-execution-approval.json
...
if not (approval_object.get("schema_version")==7 and approval_object.get("state")=="viewflow-failed-v13-abort-replay7-execution-approved" and approval_object.get("approved") is True and approval_object.get("operation_id")==op):
    raise SystemExit(67)
if not (manifest_object.get("state")=="viewflow-failed-v13-abort-replay7-command-manifest" and manifest_object.get("execution_authorized") is False and manifest_object.get("operation_id")==op):
    raise SystemExit(67)
...
def validate_live_tuple():
    ...
    if [unit_value(unit,name) for name in ("ActiveState","SubState","Transient","KillMode")]!=["active","running","yes","control-group"]:
        raise SystemExit(68)
validate_live_tuple()
exec_fd=os.memfd_create("viewflow-replay7-coordinator",os.MFD_CLOEXEC|os.MFD_ALLOW_SEALING)
...
die() { printf 'failed-v13 replay7 launcher: %s\n' "$*" >&2; exit 1; }
...
jq -e --arg op "$OPERATION_ID" --arg manifest "$MANIFEST_SHA256" --arg launcher "$launcher_sha256" \
    ...
    .schema_version == 7 and .state == "viewflow-failed-v13-abort-replay7-execution-approved" and
...
' "/proc/$$/fd/7" >/dev/null || die 'replay7 approval receipt schema/binding differs'
...
jq -e --arg op "$OPERATION_ID" --arg plan "$PLAN_SHA256" --arg prior "$PRIOR_APPROVAL_SHA256" \
    ...
    .schema_version == 1 and .state == "viewflow-failed-v13-abort-replay7-command-manifest" and
...
' "/proc/$$/fd/5" >/dev/null || die 'replay7 manifest schema/binding differs'
