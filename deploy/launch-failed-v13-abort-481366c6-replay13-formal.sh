...
readonly LAUNCHER=/home/wilf/data/viewflow/deploy/launch-failed-v13-abort-481366c6-replay13-formal.sh
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-v13-abort-481366c6-replay13-manifest.json
readonly MANIFEST_SHA256=186d4079496b7ddc092e87b32cd85bd9c711ebf64d6ac690c5b0ccf7831e1ddd
...
readonly COORDINATOR_SHA256=fb44e34b46fc440dc4763347c1578db86208960048ad940b223842cba4e88989
readonly CHECKER=/home/wilf/data/viewflow/deploy/check-cross-host-coordinator.sh
readonly CHECKER_SHA256=f3e7b909197f79705141c60828c0aacc2c932d126d297f69b2f604b4e417747a
...
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-replay13-execution-approval.json
...
if not (approval_object.get("schema_version")==13 and approval_object.get("state")=="viewflow-failed-v13-abort-replay13-execution-approved" and approval_object.get("approved") is True and approval_object.get("operation_id")==op):
    raise SystemExit(67)
if not (manifest_object.get("state")=="viewflow-failed-v13-abort-replay13-command-manifest" and manifest_object.get("execution_authorized") is False and manifest_object.get("operation_id")==op):
    raise SystemExit(67)
...
def validate_live_tuple():
    ...
    if [unit_value(unit,name) for name in ("ActiveState","SubState","Transient","KillMode")]!=["active","running","yes","control-group"]:
        raise SystemExit(68)
validate_live_tuple()
exec_fd=os.memfd_create("viewflow-replay13-coordinator",os.MFD_CLOEXEC|os.MFD_ALLOW_SEALING)
...
die() { printf 'failed-v13 replay13 launcher: %s\n' "$*" >&2; exit 1; }
...
jq -e --arg op "$OPERATION_ID" --arg manifest "$MANIFEST_SHA256" --arg launcher "$launcher_sha256" \
    ...
    .schema_version == 13 and .state == "viewflow-failed-v13-abort-replay13-execution-approved" and
...
' "/proc/$$/fd/7" >/dev/null || die 'replay13 approval receipt schema/binding differs'
...
jq -e --arg op "$OPERATION_ID" --arg plan "$PLAN_SHA256" --arg prior "$PRIOR_APPROVAL_SHA256" \
    ...
    .schema_version == 1 and .state == "viewflow-failed-v13-abort-replay13-command-manifest" and
    .execution_authorized == false and .operation_id == $op and
    .failure_class == "unique-same-run-windows-reattest-temp" and
...
' "/proc/$$/fd/5" >/dev/null || die 'replay13 manifest schema/binding differs'
