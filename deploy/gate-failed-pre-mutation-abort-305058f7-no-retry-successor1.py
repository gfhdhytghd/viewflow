...
MANIFEST_PATH = "/home/wilf/data/viewflow/deploy/failed-pre-mutation-abort-305058f7-no-retry-successor1-manifest.json"
LAUNCHER_PATH = "/home/wilf/data/viewflow/deploy/launch-failed-pre-mutation-abort-305058f7-no-retry-successor1.sh"
...
def validate_manifest(manifest, *, allow_transaction_recovery=False):
    exact_keys(manifest, {"schema_version", "state", "execution_authorized", "operation_id",
                          ...
                          "marker_generation", "threat_boundary", "terminal_inputs",
                          "candidate_replacement", "coordinator_successor", "marker",
    ...
    expected_committed = {
        ...
        "candidate_manifest": manifest["candidate_replacement"]["candidate_manifest"]["sha256"],
        "candidate_tree": manifest["candidate_replacement"]["candidate_tree_sha256"],
        "candidate_retirement_terminal": manifest["candidate_replacement"]["retirement_terminal"]["sha256"],
        "candidate_replacement_commit": manifest["candidate_replacement"]["replacement_commit"]["sha256"],
        "coordinator_successor_receipt": manifest["coordinator_successor"]["receipt"]["sha256"],
        "coordinator_successor_windows_prestate": manifest["coordinator_successor"]["windows_prestate"]["sha256"],
    }
    ...
    if not (identity.get("coordinator_instance_id") == manifest["coordinator_instance_id"]
            ...
            == manifest["windows_baseline"]["old_task_xml_sha256"]):
    ...
    replacement = manifest["candidate_replacement"]
    exact_keys(replacement, {"candidate_manifest", "candidate_tree_sha256",
                             "retirement_terminal", "replacement_commit"},
               "candidate replacement")
    successor = manifest["coordinator_successor"]
    exact_keys(successor, {"receipt", "windows_prestate", "predecessor_coordinator",
                           "predecessor_provenance", "successor_coordinator",
                           "successor_provenance", "successor_producer", "predecessor_launcher"},
               "coordinator successor")
    replacement_docs = {
        name: read_spec(replacement[name], "candidate replacement " + name, document=True)
        for name in ("candidate_manifest", "retirement_terminal", "replacement_commit")
    }
    successor_docs = {
        name: read_spec(successor[name], "coordinator successor " + name, document=True)
        for name in ("receipt", "windows_prestate")
    }
    for name in ("predecessor_coordinator", "predecessor_provenance", "successor_coordinator",
                 "successor_provenance", "successor_producer", "predecessor_launcher"):
        read_spec(successor[name], "coordinator successor " + name)
    for key, name in (("candidate_manifest", "candidate_manifest"),
                      ("candidate_replacement_commit", "replacement_commit"),
                      ("candidate_retirement_terminal", "retirement_terminal"),
                      ("coordinator_successor_receipt", "receipt"),
                      ("coordinator_successor_windows_prestate", "windows_prestate")):
        expected = replacement[name] if name in replacement else successor[name]
        if contract["inputs"].get(key) != {"path": expected["path"], "sha256": expected["sha256"]}:
            raise GateError("coordinator lineage input differs: " + key)
    if contract["inputs"].get("candidate_tree_sha256") != replacement["candidate_tree_sha256"]:
        raise GateError("coordinator candidate tree differs")
    receipt = successor_docs["receipt"]
    prestate = successor_docs["windows_prestate"]
    manifest_doc = replacement_docs["candidate_manifest"]
    retirement = replacement_docs["retirement_terminal"]
    commit = replacement_docs["replacement_commit"]
    if not (manifest_doc.get("operation_id") == OP
            and manifest_doc.get("candidate_replacement", {}).get("retirement_terminal_sha256")
            == replacement["retirement_terminal"]["sha256"]
            and retirement.get("state") == "viewflow-normal-v21-candidate-retired"
            and retirement.get("operation_id") == OP
            and commit.get("state") == "viewflow-normal-v21-candidate-replacement-committed"
            and commit.get("operation_id") == OP
            and commit.get("candidate_manifest_sha256") == replacement["candidate_manifest"]["sha256"]
            and commit.get("candidate_tree_sha256") == replacement["candidate_tree_sha256"]
            and commit.get("retirement_terminal_sha256") == replacement["retirement_terminal"]["sha256"]
            and receipt.get("schema_version") == 1
            and receipt.get("state") == "viewflow-normal-v21-coordinator-successor-authorized"
            and receipt.get("operation_id") == OP
            and receipt.get("coordinator_instance_id") == manifest["coordinator_instance_id"]
            and receipt.get("predecessor", {}).get("replacement_commit")
            == {"path": replacement["replacement_commit"]["path"],
                "sha256": replacement["replacement_commit"]["sha256"]}
            and receipt.get("predecessor", {}).get("retirement_terminal")
            == {"path": replacement["retirement_terminal"]["path"],
                "sha256": replacement["retirement_terminal"]["sha256"]}
            and receipt.get("predecessor", {}).get("candidate_manifest")
            == {"path": replacement["candidate_manifest"]["path"],
                "sha256": replacement["candidate_manifest"]["sha256"]}
            and receipt.get("predecessor", {}).get("candidate_tree_sha256")
            == replacement["candidate_tree_sha256"]
            and receipt.get("predecessor", {}).get("launcher")
            == {"path": successor["predecessor_launcher"]["path"],
                "sha256": successor["predecessor_launcher"]["sha256"]}
            and receipt.get("predecessor", {}).get("marker_handoff")
            == {"path": manifest["terminal_inputs"]["marker_handoff"]["path"],
                "sha256": manifest["terminal_inputs"]["marker_handoff"]["sha256"]}
            and receipt.get("predecessor", {}).get("linux_frozen")
            == {"path": manifest["terminal_inputs"]["linux_frozen"]["path"],
                "sha256": manifest["terminal_inputs"]["linux_frozen"]["sha256"]}
            and receipt.get("predecessor", {}).get("deployment_publish")
            == {"path": manifest["terminal_inputs"]["deployment_publish"]["path"],
                "sha256": manifest["terminal_inputs"]["deployment_publish"]["sha256"]}
            and receipt.get("windows_prestate")
            == {"path": successor["windows_prestate"]["path"],
                "sha256": successor["windows_prestate"]["sha256"]}
            and receipt.get("predecessor", {}).get("coordinator")
            == {"path": successor["predecessor_coordinator"]["path"],
                "sha256": successor["predecessor_coordinator"]["sha256"],
                "provenance_path": successor["predecessor_provenance"]["path"],
                "provenance_sha256": successor["predecessor_provenance"]["sha256"]}
            and receipt.get("successor")
            == {"coordinator_path": successor["successor_coordinator"]["path"],
                "coordinator_sha256": successor["successor_coordinator"]["sha256"],
                "provenance_path": successor["successor_provenance"]["path"],
                "provenance_sha256": successor["successor_provenance"]["sha256"],
                "receipt_producer_path": successor["successor_producer"]["path"],
                "receipt_producer_sha256": successor["successor_producer"]["sha256"]}
            and prestate.get("schema_version") == 1
            and prestate.get("state") == "viewflow-normal-v21-coordinator-successor-windows-prestate"
            and prestate.get("operation_id") == OP
            and prestate.get("operation_root_present") is False
            and prestate.get("operation_bound_task_count") == 0
            and prestate.get("operation_bound_process_count") == 0):
        raise GateError("candidate replacement/coordinator successor closure differs")
    ...
    expected_stop = {"schema_version": 1, "state": "viewflow-windows-bootstrap-stopped",
                     ...
                     "claim_sha256": "39e0485c2d418109f44bb5a6a4405aeb95172237a82e5c01807740fffe083dbf",
                     "task_name": "Viewflow Deployment " + OP,
                     "task_xml_sha256": manifest["windows_baseline"]["deployment_task_xml_sha256"],
                     "worker_pid": 6564,
                     "worker_process_start_filetime_utc": "134330059063689964",
                     "installer_process_count": 0, "task_state": "Disabled",
                     "stopped_at_utc": "2026-09-04T14:32:00.513Z"}
    ...
    repository = provenance["frozen_source_root"].rstrip("/") + "/"
    if repository != "/home/wilf/.local/state/viewflow/candidates/no-retry-v4-305058f7-20260904/frozen-source/":
        raise GateError("V4 marker frozen source root differs")
    source_pairs = [
    ...
    exact_keys(inventory_spec, {"path", "sha256", "mode", "collector", "member_count", "stable"},
               "Windows operation-root inventory spec")
    read_spec(inventory_spec["collector"], "Windows operation-root inventory collector")
    inventory = read_spec({key: inventory_spec[key] for key in ("path", "sha256", "mode")},
                          "Windows operation-root inventory", document=True)
    if not (inventory.get("schema_version") == 2
            ...
            and inventory.get("before", {}).get("operation_root", {}).get("member_count") == 14
            and len(inventory.get("before", {}).get("operation_root", {}).get("members", [])) == 14
    ...
    names = []
    for member in inventory["before"]["operation_root"]["members"]:
    ...
    exact_keys(snapshot, {"operation_root", "deployment_task", "old_peer_task", "old_peer",
                          ...
                          "mutation_outputs_present", "global_relevant_processes"},
               "Windows stable snapshot")
    old_peer = snapshot["old_peer"]
    old_task = snapshot["old_peer_task"]
    deployment_task = snapshot["deployment_task"]
    expected_member_hashes = {
        "installer-exit.json": manifest["terminal_inputs"]["installer_exit"]["sha256"],
        "linux-v13-frozen-evidence.json": manifest["terminal_inputs"]["linux_frozen"]["sha256"],
        "marker-handoff-receipt.json": manifest["terminal_inputs"]["marker_handoff"]["sha256"],
        "request.json": manifest["terminal_inputs"]["bootstrap_request"]["sha256"],
        "rollback-viewflow.ps1": manifest["windows_baseline"]["rollback_sha256"],
    }
    member_hashes = {member["name"]: member["sha256"] for member in
                     snapshot["operation_root"]["members"]}
    baseline = manifest["windows_baseline"]
    if not (all(member_hashes.get(name) == digest for name, digest in expected_member_hashes.items())
            and snapshot["bootstrap_worker_count"] == 0
            and snapshot["installer_process_count"] == 0
            and snapshot["mutation_outputs_present"] == []
            and snapshot["global_relevant_processes"] == baseline["global_relevant_processes"]
            and old_peer == {"pid": baseline["old_peer_pid"],
                             "parent_pid": baseline["old_peer_parent_pid"],
                             "start_filetime_utc": baseline["old_peer_start_filetime_utc"],
                             "session_id": 1, "sid": baseline["user_sid"],
                             "command_line_sha256": baseline["old_peer_command_line_sha256"],
                             "viewflowd_sha256": baseline["viewflowd_sha256"],
                             "wrapper_sha256": baseline["wrapper_sha256"],
                             "rollback_sha256": baseline["rollback_sha256"]}
            and old_task == {"task_name": "Viewflow Peer", "task_path": "\\",
                             "state": "Running", "xml_sha256": baseline["old_task_xml_sha256"],
                             "action_sha256": baseline["old_task_action_sha256"],
                             "principal_sha256": baseline["old_task_principal_sha256"]}
            and deployment_task.get("task_name") == baseline["deployment_task_name"]
            and deployment_task.get("task_path") == "\\"
            and deployment_task.get("state") == baseline["deployment_task_state"]
            and deployment_task.get("xml_sha256") == baseline["deployment_task_live_xml_sha256"]):
        raise GateError("Windows stable inventory/baseline closure differs")
...
def collect_windows(manifest):
    ...
    expected = {
        ...
        "operation_root_inventory": read_spec(
            ...
            "Windows operation-root inventory", document=True)["before"]["operation_root"],
    }
    if value != expected:
...
def run_sealed_marker_cli(manifest, subcommand: str, auth_sha: str) -> bytes:
    ...
    fd = os.memfd_create("viewflow-v4-marker-305", flags)
    try:
...
def validate_native_receipt(raw: bytes, manifest, auth, auth_sha: str, *, require_replay: bool):
    ...
    utc_match = UTC_NATIVE_MS.fullmatch(value.get("abort_committed_at_utc", ""))
    utc_milliseconds = None
    if utc_match is not None:
        utc_milliseconds = (
            calendar.timegm(time.strptime(utc_match.group(1), "%Y-%m-%dT%H:%M:%S")) * 1000
            + int(utc_match.group(2).ljust(3, "0"))
        )
    expected_keys = {
    ...
    if not (value["schema_version"] == 4 and value["state"] == "deployment-quarantine-aborted"
            ...
            and utc_milliseconds == int(value["abort_committed_at_unix_ms"])
...
def validate_approval(manifest, approval_sha: str, gate_sha: str, launcher_sha: str, manifest_sha: str):
    ...
    expected = {"schema_version": 4, "state": "viewflow-failed-pre-mutation-no-retry-abort-execution-approved",
                ...
                "candidate_replacement_commit_sha256": manifest["candidate_replacement"]["replacement_commit"]["sha256"],
                "coordinator_successor_receipt_sha256": manifest["coordinator_successor"]["receipt"]["sha256"],
                "coordinator_successor_windows_prestate_sha256": manifest["coordinator_successor"]["windows_prestate"]["sha256"],
...
def main():
    ...
    if args.offline_check:
        ...
        print("op305 no-retry V4 offline input/code/transaction contract passed; no SSH or mutation")
        return
    linux = linux_boundary(manifest)
    windows = collect_windows(manifest)
    if args.live_check_only:
        print("op305 no-retry V4 live boundary passed; Linux inactive and Windows old Peer unchanged; no mutation")
        return
    ...
    terminal = {"schema_version": 4, "state": "viewflow-failed-pre-mutation-no-retry-vfdqa-abort-terminal",
                ...
                "candidate_manifest_sha256": manifest["candidate_replacement"]["candidate_manifest"]["sha256"],
                "candidate_tree_sha256": manifest["candidate_replacement"]["candidate_tree_sha256"],
                "candidate_retirement_terminal_sha256": manifest["candidate_replacement"]["retirement_terminal"]["sha256"],
                "candidate_replacement_commit_sha256": manifest["candidate_replacement"]["replacement_commit"]["sha256"],
                "coordinator_successor_receipt_sha256": manifest["coordinator_successor"]["receipt"]["sha256"],
                "coordinator_successor_windows_prestate_sha256": manifest["coordinator_successor"]["windows_prestate"]["sha256"],
                "successor_authorization_consumed": True,
    ...
    print("op305 no-retry V4 VFDQA abort committed; Linux stayed inactive and Windows old Peer stayed unchanged")
...
if __name__ == "__main__":
    try:
        ...
        print(f"op305 no-retry V4 gate: {error}", file=sys.stderr)
        raise SystemExit(1)
