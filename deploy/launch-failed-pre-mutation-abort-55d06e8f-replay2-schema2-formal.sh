...
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-pre-mutation-abort-55d06e8f-replay2-schema2-manifest.json
readonly MANIFEST_SHA256=beaee6b513f5ecef3ef8597ce27369ec8ec41fb29cb6a1f5e8512b97852394ab
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/55d06e8f96aa4adc9010e53612979374/failed-pre-mutation-abort-replay2-schema2-execution-approval.json
...
expected_manifest_keys={"active_marker_gate","adopted_outputs","approval","argv","execution_authorized","fresh_outputs","immutable_terminal_inputs","installed_v13_inputs","marker_candidate","operation_id","prior_approval","required_absent","reviewed_code","schema_version","state","threat_boundary"}
if not (set(manifest)==expected_manifest_keys and manifest.get("schema_version")==2 and manifest.get("state")=="viewflow-failed-pre-mutation-abort-replay2-schema2-command-manifest" and manifest.get("execution_authorized") is False and manifest.get("operation_id")==op and manifest.get("threat_boundary")=="cooperating-crash-and-non-owner"):
    raise SystemExit(67)
...
expected_prior={"path":"/home/wilf/.local/state/viewflow/deployments/55d06e8f96aa4adc9010e53612979374/failed-pre-mutation-abort-schema2-execution-approval.json","sha256":"f7269f591e96bb518dde595c7d1b8e425d8e2155aec4ba1d32164f4299abcba4","mode":600}
...
if set(adopted)!={"linux_started"} or adopted["linux_started"]!={"path":expected_adopted_path,"sha256":"506281bd59fe982d9d8bdb7e6da6d4322b9b6aff9dfe61f2cbf0804abe728a9f","mode":600,"live_tuple":expected_live_tuple}:
    raise SystemExit(67)
entries=[]
for item in [*reviewed.values(),candidate,*terminal,*installed,prior,{key:value for key,value in adopted["linux_started"].items() if key!="live_tuple"}]:
...
if set(outputs)!=expected_output_keys or len(set(outputs.values()))!=6 or any(not isinstance(path,str) or not path.startswith("/home/wilf/.local/state/viewflow/deployments/"+op+"/") or ".replay2." not in path for path in outputs.values()):
    raise SystemExit(67)
...
approval_keys={"approved","approved_at_utc","coordinator_sha256","gate_sha256","launcher_sha256","manifest_sha256","marker_candidate_sha256","operation_id","prior_approval_sha256","publication_method","schema_version","state","terminal_state_sha256"}
expected={"schema_version":2,"state":"viewflow-failed-pre-mutation-abort-replay2-schema2-execution-approved","approved":True,"operation_id":op,"manifest_sha256":manifest_sha,"launcher_sha256":launcher_sha,"gate_sha256":gate_sha,"coordinator_sha256":reviewed["coordinator"]["sha256"],"marker_candidate_sha256":candidate["sha256"],"terminal_state_sha256":"553b49cbc0f63b5586c959e9e2e0a7e53c9a44672b7e555cb5f3602a3dc6972f","prior_approval_sha256":prior["sha256"],"publication_method":"create-once-no-replace-and-parent-fsync"}
