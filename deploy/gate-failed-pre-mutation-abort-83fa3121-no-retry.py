#!/usr/bin/env python3
"""Operation 83fa truthful no-retry V4 abort gate.
...
MANIFEST_PATH = "/home/wilf/data/viewflow/deploy/failed-pre-mutation-abort-83fa3121-no-retry-manifest.json"
LAUNCHER_PATH = "/home/wilf/data/viewflow/deploy/launch-failed-pre-mutation-abort-83fa3121-no-retry.sh"
...
def validate_manifest(manifest, *, allow_transaction_recovery=False):
    ...
    expected_stop = {"schema_version": 1, "state": "viewflow-windows-bootstrap-stopped",
                     ...
                     "claim_sha256": "a78c4e99573c4efe3221aee6c91ad4bcc82f9cf4581dad3330cb87822d171504",
                     "task_name": "Viewflow Deployment " + OP,
                     "task_xml_sha256": manifest["windows_baseline"]["deployment_task_xml_sha256"],
                     "worker_pid": 7056,
                     "worker_process_start_filetime_utc": "134327044048640609",
                     "installer_process_count": 0, "task_state": "Disabled",
                     "stopped_at_utc": "2026-09-01T02:46:59.743Z"}
...
def run_sealed_marker_cli(manifest, subcommand: str, auth_sha: str) -> bytes:
    ...
    fd = os.memfd_create("viewflow-v4-marker-83fa", flags)
    try:
...
def main():
    ...
    if args.offline_check:
        ...
        print("83fa no-retry V4 offline input/code/transaction contract passed; no SSH or mutation")
        return
    linux = linux_boundary(manifest)
    windows = collect_windows(manifest)
    if args.live_check_only:
        print("83fa no-retry V4 live boundary passed; Linux inactive and Windows old Peer unchanged; no mutation")
        return
    ...
    print("83fa no-retry V4 VFDQA abort committed; Linux stayed inactive and Windows old Peer stayed unchanged")
...
if __name__ == "__main__":
    try:
        ...
        print(f"83fa no-retry V4 gate: {error}", file=sys.stderr)
        raise SystemExit(1)
