...
readonly LAUNCHER=/home/wilf/data/viewflow/deploy/launch-failed-v13-abort-481366c6-replay6-formal.sh
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-v13-abort-481366c6-replay6-manifest.json
readonly MANIFEST_SHA256=813beb2092975616b566c7df89b7139600a9e400792cc2544e15be34e64d6d26
...
readonly COORDINATOR_SHA256=7ad80c9cc31584388c049c05cae77ccafb0f70d16ac785f56cd225ed37ccf551
readonly CHECKER=/home/wilf/data/viewflow/deploy/check-cross-host-coordinator.sh
readonly CHECKER_SHA256=673775e3418a27844af49df3d447a1c21ad9da1df4c773495c9595420f75d13f
...
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-replay6-execution-approval.json
...
if not (approval_object.get("schema_version")==6 and approval_object.get("state")=="viewflow-failed-v13-abort-replay6-execution-approved" and approval_object.get("approved") is True and approval_object.get("operation_id")==op):
    raise SystemExit(67)
if not (manifest_object.get("state")=="viewflow-failed-v13-abort-replay6-command-manifest" and manifest_object.get("execution_authorized") is False and manifest_object.get("operation_id")==op):
    raise SystemExit(67)
...
def validate_live_tuple():
    ...
    if [unit_value(unit,name) for name in ("ActiveState","SubState","Transient","KillMode")]!=["active","running","yes","control-group"]:
        raise SystemExit(68)
validate_live_tuple()
exec_fd=os.memfd_create("viewflow-replay6-coordinator",os.MFD_CLOEXEC|os.MFD_ALLOW_SEALING)
...
die() { printf 'failed-v13 replay6 launcher: %s\n' "$*" >&2; exit 1; }
...
jq -e --arg op "$OPERATION_ID" --arg manifest "$MANIFEST_SHA256" --arg launcher "$launcher_sha256" \
    ...
    .schema_version == 6 and .state == "viewflow-failed-v13-abort-replay6-execution-approved" and
...
' "/proc/$$/fd/7" >/dev/null || die 'replay6 approval receipt schema/binding differs'
...
jq -e --arg op "$OPERATION_ID" --arg plan "$PLAN_SHA256" --arg prior "$PRIOR_APPROVAL_SHA256" \
    ...
    .schema_version == 1 and .state == "viewflow-failed-v13-abort-replay6-command-manifest" and
...
' "/proc/$$/fd/5" >/dev/null || die 'replay6 manifest schema/binding differs'
