...
readonly LAUNCHER=/home/wilf/data/viewflow/deploy/launch-failed-v13-abort-481366c6-replay15-formal.sh
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-v13-abort-481366c6-replay15-manifest.json
readonly MANIFEST_SHA256=bed8833fe221b7621ac6f312fab793f5269072c41617dcd27e3c41f7f201c857
...
readonly COORDINATOR_SHA256=a669a198156d112eecd59cdad160800dce8836aec73826964de0d057695eab94
readonly CHECKER=/home/wilf/data/viewflow/deploy/check-cross-host-coordinator.sh
readonly CHECKER_SHA256=5ca036a87a14f6be412491eac3e8410ccd46447a00bcde43435b8ddeeb3e9e9c
readonly SEMANTIC_TEST=/home/wilf/data/viewflow/deploy/tests/cross-host-coordinator-semantic-test.sh
readonly SEMANTIC_TEST_SHA256=d63ecf224ea07836187eaa50d8813ea88277b20ca60f1e112de07e52689769d9
readonly NEGATIVE_TEST=/home/wilf/data/viewflow/deploy/tests/cross-host-coordinator-static-negative-test.sh
readonly NEGATIVE_TEST_SHA256=d23abfe5df340aba67dbf734c1a63d53ce47c2b9a8fb4f561b936991f5d82704
...
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-replay15-execution-approval.json
...
readonly ARCHIVED_PARTIAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/linux-v13-started.replay6-partial.json
readonly ARCHIVED_PARTIAL_SHA256=a6cb9399aeaa379f8359b9db4cb5db144d4b87d5d7fa315c07458587a00bc3c9
readonly CANONICAL_PARTIAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/linux-v13-started.json
readonly CANONICAL_PARTIAL_SHA256=a6cb9399aeaa379f8359b9db4cb5db144d4b87d5d7fa315c07458587a00bc3c9
readonly WINDOWS_PARTIAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/windows-v13-started.json
readonly WINDOWS_PARTIAL_SHA256=9426a4a22dafbc0f5283d69162bbcc04bf4819ccecde124f2628c1c0212ca763
readonly AUTHENTICATED_PARTIAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/authenticated-v13-peer.json
readonly AUTHENTICATED_PARTIAL_SHA256=606be9ef6c3788211088859e7cba31eb7a5b037f9cbceaf4cb13192f27b29a61
readonly ARCHIVED_FAILED_AUTHORIZATION=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-authorization.replay13-schema-mismatch.json
readonly ARCHIVED_FAILED_AUTHORIZATION_SHA256=8e3dc462ffd9e6ca409b99312382c40b9b42b4242245962fd1c78536277506f6
...
archived_partial,archived_partial_sha=raw[20],raw[21]
canonical_partial,canonical_partial_sha=raw[22],raw[23]
windows_partial,windows_partial_sha=raw[24],raw[25]
authenticated_partial,authenticated_partial_sha=raw[26],raw[27]
archived_failed_authorization,archived_failed_authorization_sha=raw[32],raw[33]
...
if not (approval_object.get("schema_version")==15 and approval_object.get("state")=="viewflow-failed-v13-abort-replay15-execution-approved" and approval_object.get("approved") is True and approval_object.get("operation_id")==op):
    raise SystemExit(67)
if not (manifest_object.get("state")=="viewflow-failed-v13-abort-replay15-command-manifest" and manifest_object.get("execution_authorized") is False and manifest_object.get("operation_id")==op):
    raise SystemExit(67)
archived_object=manifest_object.get("archived_partial_progress",{})
if not (archived_object.get("path")==archived_partial and archived_object.get("sha256")==archived_partial_sha and archived_object.get("canonical_output_absent") is False and archived_object.get("canonical_output_path")==canonical_partial and archived_object.get("canonical_output_sha256")==canonical_partial_sha):
    raise SystemExit(67)
if blobs[archived_partial]!=blobs[canonical_partial]:
    raise SystemExit(69)
windows_object=manifest_object.get("windows_partial_progress",{})
if not (windows_object.get("path")==windows_partial and windows_object.get("sha256")==windows_partial_sha):
    raise SystemExit(67)
authenticated_object=manifest_object.get("authenticated_partial_progress",{})
if not (authenticated_object.get("path")==authenticated_partial and authenticated_object.get("sha256")==authenticated_partial_sha):
    raise SystemExit(67)
archived_authorization_object=manifest_object.get("archived_failed_authorization",{})
if not (archived_authorization_object.get("path")==archived_failed_authorization and archived_authorization_object.get("sha256")==archived_failed_authorization_sha):
    raise SystemExit(67)
...
def validate_live_tuple():
    ...
    if [unit_value(unit,name) for name in ("ActiveState","SubState","Transient","KillMode")]!=["active","running","yes","control-group"]:
        raise SystemExit(68)
validate_live_tuple()
exec_fd=os.memfd_create("viewflow-replay15-coordinator",os.MFD_CLOEXEC|os.MFD_ALLOW_SEALING)
...
die() { printf 'failed-v13 replay15 launcher: %s\n' "$*" >&2; exit 1; }
...
exec 12<"$SEMANTIC_TEST"
exec 11<"$NEGATIVE_TEST"
exec 10<"$ARCHIVED_FAILED_AUTHORIZATION"
...
require_regular_path_fd semantic-test "$SEMANTIC_TEST" 12 "$SEMANTIC_TEST_SHA256"
require_regular_path_fd negative-test "$NEGATIVE_TEST" 11 "$NEGATIVE_TEST_SHA256"
require_regular_path_fd archived-failed-authorization "$ARCHIVED_FAILED_AUTHORIZATION" 10 "$ARCHIVED_FAILED_AUTHORIZATION_SHA256"
[[ $(stat -c '%a' -- "$ARCHIVED_FAILED_AUTHORIZATION") == 600 ]] || die 'archived failed authorization mode is not 0600'
...
launcher_sha256=$sealed_launcher_sha256
jq -e --arg op "$OPERATION_ID" --arg manifest "$MANIFEST_SHA256" --arg launcher "$launcher_sha256" \
    ...
    --arg semantic "$SEMANTIC_TEST_SHA256" --arg negative "$NEGATIVE_TEST_SHA256" \
    --arg archived_authorization "$ARCHIVED_FAILED_AUTHORIZATION_SHA256" \
    ...
    keys == ["active_marker_sha256","approved","approved_at_utc","archived_failed_authorization_sha256","checker_sha256","coordinator_sha256","launcher_sha256","negative_test_sha256","operation_id","prior_approval_sha256","publication_method","replay_manifest_sha256","schema_version","semantic_test_sha256","slot_cleanup_receipt_sha256","state","terminal_state_sha256"] and
    .schema_version == 15 and .state == "viewflow-failed-v13-abort-replay15-execution-approved" and
    ...
    .semantic_test_sha256 == $semantic and .negative_test_sha256 == $negative and
    .archived_failed_authorization_sha256 == $archived_authorization and
...
' "/proc/$$/fd/7" >/dev/null || die 'replay15 approval receipt schema/binding differs'
...
jq -e --arg op "$OPERATION_ID" --arg plan "$PLAN_SHA256" --arg prior "$PRIOR_APPROVAL_SHA256" \
    ...
    --arg semantic_path "$SEMANTIC_TEST" --arg semantic "$SEMANTIC_TEST_SHA256" \
    --arg negative_path "$NEGATIVE_TEST" --arg negative "$NEGATIVE_TEST_SHA256" \
    --arg archived_authorization_path "$ARCHIVED_FAILED_AUTHORIZATION" \
    --arg archived_authorization_sha "$ARCHIVED_FAILED_AUTHORIZATION_SHA256" \
    --arg terminal "$TERMINAL_STATE_SHA256" --arg cleanup "$SLOT_CLEANUP_SHA256" --arg marker "$ACTIVE_MARKER_SHA256" \
    --arg archived "$ARCHIVED_PARTIAL" --arg archived_sha "$ARCHIVED_PARTIAL_SHA256" \
    --arg canonical "$CANONICAL_PARTIAL" --arg canonical_sha "$CANONICAL_PARTIAL_SHA256" \
    --arg windows "$WINDOWS_PARTIAL" --arg windows_sha "$WINDOWS_PARTIAL_SHA256" \
    --arg authenticated "$AUTHENTICATED_PARTIAL" --arg authenticated_sha "$AUTHENTICATED_PARTIAL_SHA256" '
    .schema_version == 1 and .state == "viewflow-failed-v13-abort-replay15-command-manifest" and
    .execution_authorized == false and .operation_id == $op and
    .failure_class == "stale-replay13-abort-authorization" and
    ...
    .reviewed_replay.semantic_test_path == $semantic_path and .reviewed_replay.semantic_test_sha256 == $semantic and
    .reviewed_replay.negative_test_path == $negative_path and .reviewed_replay.negative_test_sha256 == $negative and
    .reviewed_replay.transient_adoption_test_sha256 == $test and
    .archived_failed_authorization.path == $archived_authorization_path and
    .archived_failed_authorization.sha256 == $archived_authorization_sha and
    .archived_partial_progress.path == $archived and
    .archived_partial_progress.sha256 == $archived_sha and
    .archived_partial_progress.canonical_output_absent == false and
    .archived_partial_progress.canonical_output_path == $canonical and
    .archived_partial_progress.canonical_output_sha256 == $canonical_sha and
    .windows_partial_progress.path == $windows and .windows_partial_progress.sha256 == $windows_sha and
    .authenticated_partial_progress.path == $authenticated and
    .authenticated_partial_progress.sha256 == $authenticated_sha and
...
' "/proc/$$/fd/5" >/dev/null || die 'replay15 manifest schema/binding differs'
...
require_plan_artifact archived-partial "$ARCHIVED_PARTIAL" "$ARCHIVED_PARTIAL_SHA256"
require_plan_artifact canonical-partial "$CANONICAL_PARTIAL" "$CANONICAL_PARTIAL_SHA256"
require_plan_artifact windows-partial "$WINDOWS_PARTIAL" "$WINDOWS_PARTIAL_SHA256"
require_plan_artifact authenticated-partial "$AUTHENTICATED_PARTIAL" "$AUTHENTICATED_PARTIAL_SHA256"
require_plan_artifact archived-failed-authorization "$ARCHIVED_FAILED_AUTHORIZATION" "$ARCHIVED_FAILED_AUTHORIZATION_SHA256"
...
done < <(jq -r --arg linux "$CANONICAL_PARTIAL" --arg windows "$WINDOWS_PARTIAL" \
    --arg authenticated "$AUTHENTICATED_PARTIAL" \
    '(.required_absent_branch_sentinels[],.fresh_outputs[]) | select(. != $linux and . != $windows and . != $authenticated)' "/proc/$$/fd/8")

validate_live_adoption
/usr/bin/bash "/proc/$$/fd/6" "$COORDINATOR" >/dev/null
/usr/bin/bash "/proc/$$/fd/3" "$COORDINATOR" >/dev/null
CROSS_HOST_COORDINATOR_CHECKER="/proc/$$/fd/6" \
    /usr/bin/bash "/proc/$$/fd/12" "$COORDINATOR" >/dev/null
CROSS_HOST_COORDINATOR_SOURCE="$COORDINATOR" CROSS_HOST_COORDINATOR_CHECKER="/proc/$$/fd/6" \
    CROSS_HOST_COORDINATOR_SEMANTIC_TEST="/proc/$$/fd/12" \
    /usr/bin/bash "/proc/$$/fd/11" >/dev/null
require_regular_path_fd coordinator "$COORDINATOR" 9 "$COORDINATOR_SHA256"
require_regular_path_fd plan "$PLAN" 8 "$PLAN_SHA256"
require_regular_path_fd approval "$APPROVAL" 7 "$approval_sha256"
require_regular_path_fd checker "$CHECKER" 6 "$CHECKER_SHA256"
...
exec "$GATE_ENV" -i HOME=/home/wilf PATH=/usr/bin:/bin \
    ...
    "$ARCHIVED_PARTIAL" "$ARCHIVED_PARTIAL_SHA256" "$CANONICAL_PARTIAL" "$CANONICAL_PARTIAL_SHA256" \
    "$WINDOWS_PARTIAL" "$WINDOWS_PARTIAL_SHA256" \
    "$AUTHENTICATED_PARTIAL" "$AUTHENTICATED_PARTIAL_SHA256" \
    "$SEMANTIC_TEST" "$SEMANTIC_TEST_SHA256" \
    "$NEGATIVE_TEST" "$NEGATIVE_TEST_SHA256" \
    "$ARCHIVED_FAILED_AUTHORIZATION" "$ARCHIVED_FAILED_AUTHORIZATION_SHA256"
