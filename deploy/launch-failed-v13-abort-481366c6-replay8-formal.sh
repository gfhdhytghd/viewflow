...
readonly LAUNCHER=/home/wilf/data/viewflow/deploy/launch-failed-v13-abort-481366c6-replay8-formal.sh
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-v13-abort-481366c6-replay8-manifest.json
...
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-replay8-execution-approval.json
...
if not (approval_object.get("schema_version")==8 and approval_object.get("state")=="viewflow-failed-v13-abort-replay8-execution-approved" and approval_object.get("approved") is True and approval_object.get("operation_id")==op):
    raise SystemExit(67)
if not (manifest_object.get("state")=="viewflow-failed-v13-abort-replay8-command-manifest" and manifest_object.get("execution_authorized") is False and manifest_object.get("operation_id")==op):
    raise SystemExit(67)
archived_object=manifest_object.get("archived_partial_progress",{})
if not (archived_object.get("path")==archived_partial and archived_object.get("sha256")==archived_partial_sha and archived_object.get("canonical_output_absent") is True):
    raise SystemExit(67)
canonical_output=os.path.join(os.path.dirname(archived_partial),"linux-v13-started.json")
...
def validate_live_tuple():
    ...
    if [unit_value(unit,name) for name in ("ActiveState","SubState","Transient","KillMode")]!=["active","running","yes","control-group"]:
        raise SystemExit(68)
validate_live_tuple()
exec_fd=os.memfd_create("viewflow-replay8-coordinator",os.MFD_CLOEXEC|os.MFD_ALLOW_SEALING)
...
die() { printf 'failed-v13 replay8 launcher: %s\n' "$*" >&2; exit 1; }
...
jq -e --arg op "$OPERATION_ID" --arg manifest "$MANIFEST_SHA256" --arg launcher "$launcher_sha256" \
    ...
    .schema_version == 8 and .state == "viewflow-failed-v13-abort-replay8-execution-approved" and
...
' "/proc/$$/fd/7" >/dev/null || die 'replay8 approval receipt schema/binding differs'
...
jq -e --arg op "$OPERATION_ID" --arg plan "$PLAN_SHA256" --arg prior "$PRIOR_APPROVAL_SHA256" \
    ...
    .schema_version == 1 and .state == "viewflow-failed-v13-abort-replay8-command-manifest" and
    ...
    .archived_partial_progress.canonical_output_absent == true and
...
' "/proc/$$/fd/5" >/dev/null || die 'replay8 manifest schema/binding differs'
...
[[ ! -e /home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/linux-v13-started.json &&
   ! -L /home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/linux-v13-started.json ]] ||
    die 'canonical partial-progress output reappeared'
...
exec "$GATE_ENV" -i HOME=/home/wilf PATH=/usr/bin:/bin \
    ...
    "$ARCHIVED_PARTIAL" "$ARCHIVED_PARTIAL_SHA256"
