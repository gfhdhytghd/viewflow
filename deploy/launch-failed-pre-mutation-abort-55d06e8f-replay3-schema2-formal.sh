...
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-pre-mutation-abort-55d06e8f-replay3-schema2-manifest.json
readonly MANIFEST_SHA256=639b6b32b6e3a70fda5d9cb77e9cef447f7a94370d9a5168534c8ab13170e7be
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/55d06e8f96aa4adc9010e53612979374/failed-pre-mutation-abort-replay3-schema2-execution-approval.json
...
expected_manifest_keys={"active_marker_gate","adopted_outputs","approval","argv","execution_authorized","fresh_outputs","immutable_terminal_inputs","initial_approval","installed_v13_inputs","marker_candidate","operation_id","prior_approval","required_absent","reviewed_code","schema_version","state","threat_boundary"}
if not (set(manifest)==expected_manifest_keys and manifest.get("schema_version")==2 and manifest.get("state")=="viewflow-failed-pre-mutation-abort-replay3-schema2-command-manifest" and manifest.get("execution_authorized") is False and manifest.get("operation_id")==op and manifest.get("threat_boundary")=="cooperating-crash-and-non-owner"):
    raise SystemExit(67)
...
if set(outputs)!=expected_output_keys or len(set(outputs.values()))!=6 or any(not isinstance(path,str) or not path.startswith("/home/wilf/.local/state/viewflow/deployments/"+op+"/") or ".replay3." not in path for path in outputs.values()):
    raise SystemExit(67)
