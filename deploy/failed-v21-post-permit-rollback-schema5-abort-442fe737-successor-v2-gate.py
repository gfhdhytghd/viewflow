#!/usr/bin/env python3
"""Successor-v2 sealed gate for the honest schema-5 abort of operation 442fe737.
...
OLD_OP = "845ce4223e4f426a8a0015d3355595e8"
ROOT = Path("/home/wilf/.local/state/viewflow/deployments") / OP
MANIFEST = "/home/wilf/data/viewflow/deploy/failed-v21-post-permit-rollback-schema5-abort-442fe737-successor-v2-manifest.json"
...
def validate_manifest(value, active_marker_required=True, execution_approval_may_exist=False):
    exact_keys(value, {"schema_version", "state", "execution_authorized", "operation_id",
                       ...
                       "approval_path", "outputs", "argv", "failed_attempt_v1"},
               "manifest")
    if not (value["schema_version"] == 2
            and value["state"] == "viewflow-failed-v21-post-permit-rollback-schema5-abort-442fe737-successor-v2-command-manifest"
            ...
            and value["recovery_boundary"] == {"phase": "WINDOWS_ROLLED_BACK",
                "failure_phase": "MUTATION_PERMITTED", "mutation_possible": True,
                "force_release_executed": False, "rollback_performed": True}):
        raise GateError("manifest identity or truthful rollback boundary differs")
    failed = value["failed_attempt_v1"]
    exact_keys(failed, {"schema_version", "state", "failure", "mutation_started",
                        "approval", "sealed_set"}, "failed attempt v1")
    exact_keys(failed["sealed_set"], {"manifest", "gate", "launcher", "publisher",
        "checker", "hermetic", "offline", "negative"}, "failed attempt v1 sealed set")
    failed_documents = {}
    for name, spec in failed["sealed_set"].items():
        failed_documents[name] = read_spec(spec, "failed attempt v1 " + name, False)
    failed_approval_raw = read_spec(failed["approval"], "failed attempt v1 approval", False)
    failed_approval = strict_json(failed_approval_raw, "failed attempt v1 approval")
    if not (failed["schema_version"] == 1
            and failed["state"] == "viewflow-op442-schema5-abort-pre-mutation-approval-gate-failed"
            and failed["failure"] == "execution-approval-remained-in-required-absent"
            and failed["mutation_started"] is False
            and failed["approval"]["path"] != value["approval_path"]
            and failed_approval.get("schema_version") == 1
            and failed_approval.get("state")
                == "viewflow-failed-v21-post-permit-rollback-schema5-abort-442fe737-execution-approved"
            and failed_approval.get("approved") is True
            and failed_approval.get("operation_id") == OP
            and failed_approval.get("manifest_sha256")
                == failed["sealed_set"]["manifest"]["sha256"]
            and failed_approval.get("gate_sha256") == failed["sealed_set"]["gate"]["sha256"]
            and failed_approval.get("launcher_sha256")
                == failed["sealed_set"]["launcher"]["sha256"]
            and digest(failed_documents["manifest"])
                == failed["sealed_set"]["manifest"]["sha256"]):
        raise GateError("failed attempt v1 audit evidence differs")
    ...
    exact_keys(coordinator, {"path", "sha256", "mode", "size", "checker_path",
                             ...
                             "transient_adoption_test_path", "transient_adoption_test_sha256",
                             "transient_adoption_test_size"},
               "coordinator")
    stable_read(coordinator["path"], coordinator["sha256"], 0o755,
                coordinator["size"], "coordinator")
    for stem in ("checker", "semantic_test", "negative_test", "transient_adoption_test"):
        path = coordinator[stem + "_path"]
        stable_read(path, coordinator[stem + "_sha256"], 0o755,
                    coordinator[stem + "_size"], stem)
    expected_inputs = {"schema1_handoff_lineage", "coordinator_state", "marker_handoff",
        "deployment_publish", "linux_frozen", "windows_request", "windows_prepared",
        "mutation_permit", "installer_exit", "windows_stop", "recovery_bundle",
        "linux_deactivation", "linux_deactivation_transcript", "windows_rollback",
        "candidate_manifest", "old_schema1_terminal", "old_schema1_authorization",
        "old_schema1_abort", "old_schema1_query", "old_schema1_transition",
        "old_schema1_handoff", "old_schema1_vfdqa", "marker_cli_candidate",
    ...
    expected_committed = {"marker_handoff": "marker_handoff", "linux_frozen": "linux_frozen",
        ...
        "windows_exit": "installer_exit", "windows_stop_evidence": "windows_stop",
    ...
    if not (state.get("operation_id") == OP and state.get("phase") == "WINDOWS_ROLLED_BACK"
            and state.get("recovery") == {"failure_phase": "MUTATION_PERMITTED",
    ...
    lineage = documents["schema1_handoff_lineage"]
    old = lineage.get("old_schema1_abort", {})
    if not (lineage.get("schema_version") == 1
            and lineage.get("state") == "viewflow-schema1-abort-handoff-to-fresh-v21"
            and lineage.get("old_operation_id") == OLD_OP and lineage.get("new_operation_id") == OP
            and old == {"terminal_sha256": value["immutable_inputs"]["old_schema1_terminal"]["sha256"],
                        "authorization_sha256": value["immutable_inputs"]["old_schema1_authorization"]["sha256"],
                        "abort_receipt_sha256": value["immutable_inputs"]["old_schema1_abort"]["sha256"],
                        "abort_query_sha256": value["immutable_inputs"]["old_schema1_query"]["sha256"],
                        "transition_sha256": value["immutable_inputs"]["old_schema1_transition"]["sha256"],
                        "handoff_persistent_sha256": value["immutable_inputs"]["old_schema1_handoff"]["sha256"],
                        "vfdqa_sha256": value["immutable_inputs"]["old_schema1_vfdqa"]["sha256"]}):
        raise GateError("schema1 handoff lineage differs")
    old_vfdqa = documents["old_schema1_vfdqa"]
    if not (len(old_vfdqa) == 384 and old_vfdqa[:8] == b"VFDQA001"
            and hashlib.sha256(old_vfdqa[:352]).digest() == old_vfdqa[352:]):
        raise GateError("old schema1 VFDQA closure differs")
    provenance = documents["marker_cli_provenance"]
    candidate_spec = value["immutable_inputs"]["marker_cli_candidate"]
    if not (provenance.get("operation_id") == OP
            and provenance.get("candidate", {}).get("path") == candidate_spec["path"]
            and provenance.get("candidate", {}).get("sha256") == candidate_spec["sha256"]
            and provenance.get("contract", {}).get("force_release_executed") is False
        ...
        raise GateError("marker schema5 candidate provenance differs")
    ...
    expected_receipts = [
        ...
        {"name": "installer-exit.json", "sha256": value["immutable_inputs"]["installer_exit"]["sha256"]},
    ...
    if not (windows["ssh_target"] == "wilf@172.16.105.70"
            ...
            and all(HEX.fullmatch(str(windows[key])) for key in
                    ("old_task_xml_sha256", "old_binary_sha256", "old_wrapper_sha256",
                     "rollback_sha256", "deployment_task_xml_sha256"))):
    ...
    if len(set(outputs.values())) != len(outputs) or any(
            not path.startswith(str(ROOT) + "/schema5-abort-442fe737-")
            for path in outputs.values()):
        raise GateError("output path confinement differs")
    options = argv_map(value["argv"])
    exact_keys(options, {"--abort-failed-v13", "--schema1-handoff-lineage-receipt",
        "--schema1-handoff-lineage-receipt-sha256",
        "--schema1-handoff-abort-marker-cli-candidate",
        "--schema1-handoff-abort-marker-cli-sha256", "--failed-v13-original-generation-only",
        ...
        "--windows-installer-exit-receipt", "--windows-force-release-envelope",
        "--local-windows-rollback-receipt", "--linux-deactivation-transcript",
    ...
    required = {"--schema1-handoff-lineage-receipt": value["immutable_inputs"]["schema1_handoff_lineage"]["path"],
        "--schema1-handoff-lineage-receipt-sha256": value["immutable_inputs"]["schema1_handoff_lineage"]["sha256"],
        "--schema1-handoff-abort-marker-cli-candidate": candidate_spec["path"],
        "--schema1-handoff-abort-marker-cli-sha256": candidate_spec["sha256"],
    ...
    expected_absent = [
        ...
        str(ROOT / "windows-force-envelope.json"),
    ...
    if (value["required_absent"] != expected_absent
            ...
            or failed["approval"]["path"] in value["required_absent"]):
...
def linux_live_census(manifest):
    ...
    return {"schema_version": 1, "state": "viewflow-op442-linux-live-census",
        "operation_id": OP,
...
def validate_linux_live_census(value):
    ...
    if not (value["schema_version"] == 1 and value["state"] == "viewflow-op442-linux-live-census"
...
def validate_windows_live_census(value, manifest):
    ...
    windows = manifest["windows_expected"]
    if not (value["schema_version"] == 1 and value["state"] == "viewflow-op442-windows-live-census"
...
def run_coordinator(manifest):
    ...
    fd = seal(raw, "viewflow-schema5-abort-coordinator-442fe737")
    try:
...
def run_query(manifest, authorization_sha):
    ...
    fd = seal(raw, "viewflow-schema5-marker-query-442fe737")
    try:
        ...
        if result.returncode != 0 or result.stderr:
            raise GateError("sealed schema5 marker query failed")
        strict_json(result.stdout, "schema5 abort query")
...
def validate_authorization(raw: bytes, manifest):
    value = strict_json(raw, "schema5 authorization")
    exact_keys(value, {"authenticated_v13_peer_receipt_sha256", "authorization_receipt_path",
        ...
        "initial_force_release_executed", "installer_exit_receipt_sha256",
        "linux_deactivation_proof_sha256", "linux_deactivation_transcript_sha256",
        "linux_frozen_evidence_sha256", "linux_v13_started_receipt_sha256",
        ...
        "schema1_handoff_lineage_receipt_sha256", "schema_version",
        "second_force_release_executed", "state", "windows_prepared_receipt_sha256",
        "windows_rollback_receipt_sha256", "windows_stop_evidence_sha256",
        "windows_v13_started_receipt_sha256"}, "schema5 authorization")
    inputs = manifest["immutable_inputs"]
    bindings = {
        "coordinator_terminal_state_sha256": "coordinator_state",
        "schema1_handoff_lineage_receipt_sha256": "schema1_handoff_lineage",
        ...
        "installer_exit_receipt_sha256": "installer_exit",
    ...
    if not (value.get("schema_version") == 5
            and value.get("state") == "viewflow-deployment-quarantine-post-permit-rollback-abort-authorized"
            ...
            and value.get("coordinator_failure_phase") == "MUTATION_PERMITTED"
            and value.get("coordinator_mutation_possible") is True
            and value.get("mutation_permit_published") is True
            and value.get("force_release_executed") is False
            and value.get("rollback_performed") is True
            and value.get("initial_force_release_executed") is False
        ...
        raise GateError("schema5 authorization truth/binding differs")
    return value


def validate_receipt(raw: bytes, manifest, authorization_sha: str, replayed: bool):
    value = strict_json(raw, "schema5 abort receipt")
    exact_keys(value, {"abort_authorization_path", "abort_authorization_sha256",
        ...
        "installer_exit_receipt_sha256", "linux_deactivation_proof_sha256",
        "linux_deactivation_transcript_sha256", "linux_frozen_evidence_sha256",
        "linux_v13_started_receipt_sha256", "marker_created_at_unix_ms",
        ...
        "rollback_performed", "rollback_token_consumed",
        "schema1_handoff_lineage_receipt_sha256", "schema_version",
        "second_force_release_executed", "source_display_id", "state", "target_device_id",
        "windows_prepared_receipt_sha256", "windows_rollback_receipt_sha256",
    ...
    if not (value.get("schema_version") == 5 and value.get("state") == "deployment-quarantine-aborted"
            and value.get("authorization_state") == "viewflow-deployment-quarantine-post-permit-rollback-abort-authorized"
        ...
        raise GateError("schema5 abort receipt truth/binding differs")
    return value
...
def validate_approval(manifest, raw, manifest_sha, gate_sha, launcher_sha):
    ...
    expected = {"schema_version": 2,
        ...
        "lineage_sha256": manifest["immutable_inputs"]["schema1_handoff_lineage"]["sha256"],
...
def main():
    ...
    if args.offline_check:
        print("442fe737 schema5 abort successor-v2 offline contract passed; no SSH or mutation")
        return
    if args.live_check_only:
        ...
        print("442fe737 schema5 abort successor-v2 cross-host readonly boundary passed; no mutation")
        return
    ...
    if {key: value for key, value in receipt.items() if key != "replayed"} != {
        ...
        raise GateError("schema5 abort query differs from committed receipt")
    ...
    if not os.path.lexists(manifest["outputs"]["abort_query"]):
        create_once(manifest["outputs"]["abort_query"], query_raw)
                               ...
                               "persisted schema5 abort query") != query_raw:
        raise GateError("persisted schema5 abort query differs")
    vfdqa_sha, retired = validate_post_state(manifest, receipt, auth_sha)
    terminal = {"schema_version": 1,
        "state": "viewflow-failed-v21-post-permit-rollback-schema5-vfdqa-abort-terminal",
        ...
        "coordinator_sha256": manifest["coordinator"]["sha256"],
        "coordinator_state_sha256": manifest["immutable_inputs"]["coordinator_state"]["sha256"],
        "schema1_handoff_lineage_sha256": manifest["immutable_inputs"]["schema1_handoff_lineage"]["sha256"],
        "marker_cli_candidate_sha256": manifest["immutable_inputs"]["marker_cli_candidate"]["sha256"],
    ...
    if terminal_exists:
        existing = stable_generated_read(manifest["outputs"]["terminal"],
                                         "schema5 abort terminal")
        if existing != terminal_raw:
            raise GateError("existing terminal differs")
        print("442fe737 schema5 VFDQA abort terminal reattested; no redispatch")
    else:
        create_once(manifest["outputs"]["terminal"], terminal_raw)
        print("442fe737 schema5 VFDQA abort terminal committed")
...
if __name__ == "__main__":
    try:
        ...
        print("442fe737 schema5 abort successor-v2 gate: " + str(error), file=sys.stderr)
        raise SystemExit(1)
