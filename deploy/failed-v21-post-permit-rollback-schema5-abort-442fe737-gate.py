#!/usr/bin/env python3
"""Sealed gate for the honest schema-5 abort of operation 442fe737.
...
def validate_manifest(value, active_marker_required=True):
    exact_keys(value, {"schema_version", "state", "execution_authorized", "operation_id",
                       ...
                       "approval_path", "outputs", "argv"},
               "manifest")
    if not (value["schema_version"] == 1
            and value["state"] == "viewflow-failed-v21-post-permit-rollback-schema5-abort-442fe737-command-manifest"
    ...
    if not isinstance(value["required_absent"], list) or len(value["required_absent"]) != len(set(value["required_absent"])):
...
def main():
    ...
    validate_manifest(manifest, active_required)
    ...
    if args.offline_check:
        print("442fe737 schema5 abort offline contract passed; no SSH or mutation")
        return
    if args.live_check_only:
        ...
        print("442fe737 schema5 abort cross-host readonly boundary passed; no mutation")
        return
...
if __name__ == "__main__":
    try:
        ...
        print("442fe737 schema5 abort gate: " + str(error), file=sys.stderr)
        raise SystemExit(1)
