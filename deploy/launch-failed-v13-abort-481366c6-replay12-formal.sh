...
readonly LAUNCHER=/home/wilf/data/viewflow/deploy/launch-failed-v13-abort-481366c6-replay12-formal.sh
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-v13-abort-481366c6-replay12-manifest.json
readonly MANIFEST_SHA256=c81418cfa02faf502706b416f72e52c9734e548265548e3c2d84031279c02c9b
...
readonly COORDINATOR_SHA256=5ff6c1817dda49821b1906f49520d0b9a6d6fce757219913f787106c225fa987
readonly CHECKER=/home/wilf/data/viewflow/deploy/check-cross-host-coordinator.sh
readonly CHECKER_SHA256=2ed538db11bb46c06709bd96d1a69abbadb9b400cbb2aa1a50012c7fa9ddf61a
...
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-replay12-execution-approval.json
...
if not (approval_object.get("schema_version")==12 and approval_object.get("state")=="viewflow-failed-v13-abort-replay12-execution-approved" and approval_object.get("approved") is True and approval_object.get("operation_id")==op):
    raise SystemExit(67)
if not (manifest_object.get("state")=="viewflow-failed-v13-abort-replay12-command-manifest" and manifest_object.get("execution_authorized") is False and manifest_object.get("operation_id")==op):
    raise SystemExit(67)
...
def validate_live_tuple():
    ...
    if [unit_value(unit,name) for name in ("ActiveState","SubState","Transient","KillMode")]!=["active","running","yes","control-group"]:
        raise SystemExit(68)
validate_live_tuple()
exec_fd=os.memfd_create("viewflow-replay12-coordinator",os.MFD_CLOEXEC|os.MFD_ALLOW_SEALING)
...
die() { printf 'failed-v13 replay12 launcher: %s\n' "$*" >&2; exit 1; }
...
jq -e --arg op "$OPERATION_ID" --arg manifest "$MANIFEST_SHA256" --arg launcher "$launcher_sha256" \
    ...
    .schema_version == 12 and .state == "viewflow-failed-v13-abort-replay12-execution-approved" and
...
' "/proc/$$/fd/7" >/dev/null || die 'replay12 approval receipt schema/binding differs'
...
jq -e --arg op "$OPERATION_ID" --arg plan "$PLAN_SHA256" --arg prior "$PRIOR_APPROVAL_SHA256" \
    ...
    .schema_version == 1 and .state == "viewflow-failed-v13-abort-replay12-command-manifest" and
    .execution_authorized == false and .operation_id == $op and
    .failure_class == "same-run-windows-receipt-live-reattest-resume" and
...
' "/proc/$$/fd/5" >/dev/null || die 'replay12 manifest schema/binding differs'
