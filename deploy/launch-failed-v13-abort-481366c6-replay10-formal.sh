...
readonly LAUNCHER=/home/wilf/data/viewflow/deploy/launch-failed-v13-abort-481366c6-replay10-formal.sh
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-v13-abort-481366c6-replay10-manifest.json
readonly MANIFEST_SHA256=4e8facd9c055793bcb56c7e47d8dd9daffc6146876591129c745d332d94e3b10
...
readonly COORDINATOR_SHA256=15fb8fc5889526b3e90e8b923806e52bb95ba59191a98f4530b2aa331e1fc355
readonly CHECKER=/home/wilf/data/viewflow/deploy/check-cross-host-coordinator.sh
readonly CHECKER_SHA256=e4ca4066f71807e96d63d05bb95b95625d205a8dd3a20c0a77138a0823b6127f
...
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-replay10-execution-approval.json
...
if not (approval_object.get("schema_version")==10 and approval_object.get("state")=="viewflow-failed-v13-abort-replay10-execution-approved" and approval_object.get("approved") is True and approval_object.get("operation_id")==op):
    raise SystemExit(67)
if not (manifest_object.get("state")=="viewflow-failed-v13-abort-replay10-command-manifest" and manifest_object.get("execution_authorized") is False and manifest_object.get("operation_id")==op):
    raise SystemExit(67)
...
def validate_live_tuple():
    ...
    if [unit_value(unit,name) for name in ("ActiveState","SubState","Transient","KillMode")]!=["active","running","yes","control-group"]:
        raise SystemExit(68)
validate_live_tuple()
exec_fd=os.memfd_create("viewflow-replay10-coordinator",os.MFD_CLOEXEC|os.MFD_ALLOW_SEALING)
...
die() { printf 'failed-v13 replay10 launcher: %s\n' "$*" >&2; exit 1; }
...
jq -e --arg op "$OPERATION_ID" --arg manifest "$MANIFEST_SHA256" --arg launcher "$launcher_sha256" \
    ...
    .schema_version == 10 and .state == "viewflow-failed-v13-abort-replay10-execution-approved" and
...
' "/proc/$$/fd/7" >/dev/null || die 'replay10 approval receipt schema/binding differs'
...
jq -e --arg op "$OPERATION_ID" --arg plan "$PLAN_SHA256" --arg prior "$PRIOR_APPROVAL_SHA256" \
    ...
    .schema_version == 1 and .state == "viewflow-failed-v13-abort-replay10-command-manifest" and
    .execution_authorized == false and .operation_id == $op and
    .failure_class == "replay9-launcher-stale-source-pins" and
...
' "/proc/$$/fd/5" >/dev/null || die 'replay10 manifest schema/binding differs'
...
done < <(jq -r --arg partial "$CANONICAL_PARTIAL" \
    '(.required_absent_branch_sentinels[],.fresh_outputs[]) | select(. != $partial)' "/proc/$$/fd/8")
