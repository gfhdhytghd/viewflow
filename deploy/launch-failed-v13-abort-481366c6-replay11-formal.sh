...
readonly LAUNCHER=/home/wilf/data/viewflow/deploy/launch-failed-v13-abort-481366c6-replay11-formal.sh
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-v13-abort-481366c6-replay11-manifest.json
readonly MANIFEST_SHA256=b6ffd0e0a52120dfed0783d29004f8b8176425c2d334ec5d54d282a7db3da0d7
...
readonly COORDINATOR_SHA256=bff7410f6f179f68272e5d83ba7b97a6cb02c0a7cc0a9b4887d75c44cebd8726
readonly CHECKER=/home/wilf/data/viewflow/deploy/check-cross-host-coordinator.sh
readonly CHECKER_SHA256=096c99379622ca2a354dd2c7afddefb83c320769a4b1f20bf5538601b612c8d4
...
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-replay11-execution-approval.json
...
if not (approval_object.get("schema_version")==11 and approval_object.get("state")=="viewflow-failed-v13-abort-replay11-execution-approved" and approval_object.get("approved") is True and approval_object.get("operation_id")==op):
    raise SystemExit(67)
if not (manifest_object.get("state")=="viewflow-failed-v13-abort-replay11-command-manifest" and manifest_object.get("execution_authorized") is False and manifest_object.get("operation_id")==op):
    raise SystemExit(67)
...
def validate_live_tuple():
    ...
    if [unit_value(unit,name) for name in ("ActiveState","SubState","Transient","KillMode")]!=["active","running","yes","control-group"]:
        raise SystemExit(68)
validate_live_tuple()
exec_fd=os.memfd_create("viewflow-replay11-coordinator",os.MFD_CLOEXEC|os.MFD_ALLOW_SEALING)
...
die() { printf 'failed-v13 replay11 launcher: %s\n' "$*" >&2; exit 1; }
...
jq -e --arg op "$OPERATION_ID" --arg manifest "$MANIFEST_SHA256" --arg launcher "$launcher_sha256" \
    ...
    .schema_version == 11 and .state == "viewflow-failed-v13-abort-replay11-execution-approved" and
...
' "/proc/$$/fd/7" >/dev/null || die 'replay11 approval receipt schema/binding differs'
...
jq -e --arg op "$OPERATION_ID" --arg plan "$PLAN_SHA256" --arg prior "$PRIOR_APPROVAL_SHA256" \
    ...
    .schema_version == 1 and .state == "viewflow-failed-v13-abort-replay11-command-manifest" and
    .execution_authorized == false and .operation_id == $op and
    .failure_class == "windows-auth-source-live-reattest-and-resume-safe-probe" and
...
' "/proc/$$/fd/5" >/dev/null || die 'replay11 manifest schema/binding differs'
...
while IFS= read -r artifact_path; do
    ...
    '(.required_absent_branch_sentinels[],.fresh_outputs[]) | select(. != $linux and . != $windows)' "/proc/$$/fd/8")
