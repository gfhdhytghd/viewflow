...
readonly LAUNCHER=/home/wilf/data/viewflow/deploy/launch-failed-v13-abort-481366c6-replay14-formal.sh
readonly MANIFEST=/home/wilf/data/viewflow/deploy/failed-v13-abort-481366c6-replay14-manifest.json
readonly MANIFEST_SHA256=1fbbbe414190a09ce04196a4b7265fff5a084f4ade043c1eb941962a85fc7f4e
...
readonly APPROVAL=/home/wilf/.local/state/viewflow/deployments/481366c6c9654a679a96d4b6b1447ecb/failed-v13-abort-replay14-execution-approval.json
...
if not (approval_object.get("schema_version")==14 and approval_object.get("state")=="viewflow-failed-v13-abort-replay14-execution-approved" and approval_object.get("approved") is True and approval_object.get("operation_id")==op):
    raise SystemExit(67)
if not (manifest_object.get("state")=="viewflow-failed-v13-abort-replay14-command-manifest" and manifest_object.get("execution_authorized") is False and manifest_object.get("operation_id")==op):
    raise SystemExit(67)
...
def validate_live_tuple():
    ...
    if [unit_value(unit,name) for name in ("ActiveState","SubState","Transient","KillMode")]!=["active","running","yes","control-group"]:
        raise SystemExit(68)
validate_live_tuple()
exec_fd=os.memfd_create("viewflow-replay14-coordinator",os.MFD_CLOEXEC|os.MFD_ALLOW_SEALING)
...
die() { printf 'failed-v13 replay14 launcher: %s\n' "$*" >&2; exit 1; }
...
jq -e --arg op "$OPERATION_ID" --arg manifest "$MANIFEST_SHA256" --arg launcher "$launcher_sha256" \
    ...
    keys == ["active_marker_sha256","approved","approved_at_utc","checker_sha256","coordinator_sha256","launcher_sha256","negative_test_sha256","operation_id","prior_approval_sha256","publication_method","replay_manifest_sha256","schema_version","semantic_test_sha256","slot_cleanup_receipt_sha256","state","terminal_state_sha256"] and
    .schema_version == 14 and .state == "viewflow-failed-v13-abort-replay14-execution-approved" and
...
' "/proc/$$/fd/7" >/dev/null || die 'replay14 approval receipt schema/binding differs'
...
jq -e --arg op "$OPERATION_ID" --arg plan "$PLAN_SHA256" --arg prior "$PRIOR_APPROVAL_SHA256" \
    ...
    .schema_version == 1 and .state == "viewflow-failed-v13-abort-replay14-command-manifest" and
    .execution_authorized == false and .operation_id == $op and
    .failure_class == "marker-auth-installed-cli-schema" and
...
' "/proc/$$/fd/5" >/dev/null || die 'replay14 manifest schema/binding differs'
