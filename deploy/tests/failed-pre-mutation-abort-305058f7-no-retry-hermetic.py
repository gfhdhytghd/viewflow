...
GATE = "/home/wilf/data/viewflow/deploy/gate-failed-pre-mutation-abort-305058f7-no-retry-successor1.py"
...
approval_spec = importlib.util.spec_from_file_location(
    "vf_v4_approval",
    "/home/wilf/data/viewflow/deploy/publish-failed-pre-mutation-abort-305058f7-no-retry-successor1-approval.py",
)
approval = importlib.util.module_from_spec(approval_spec)
approval_spec.loader.exec_module(approval)

manifest = json.loads(Path(
    "/home/wilf/data/viewflow/deploy/failed-pre-mutation-abort-305058f7-no-retry-successor1-manifest.json"
).read_text())
module.validate_manifest(manifest)
state = json.loads(Path(manifest["terminal_inputs"]["coordinator_state"]["path"]).read_text())
assert state["phase"] == "LINUX_RECOVERED"
assert state["recovery"] == {"failure_phase": "WINDOWS_STARTED", "mutation_possible": False}
assert set(state["committed_artifacts"]) == {
    "marker_handoff", "linux_frozen", "publish_receipt", "bootstrap_request",
    "windows_exit", "windows_stop_evidence", "candidate_manifest", "candidate_tree",
    "candidate_retirement_terminal", "candidate_replacement_commit",
    "coordinator_successor_receipt", "coordinator_successor_windows_prestate",
}
...
assert "coordinator_successor_receipt_sha256" not in authorization
assert manifest["coordinator_successor"]["receipt"]["sha256"] == state["committed_artifacts"]["coordinator_successor_receipt"]
...
with tempfile.TemporaryDirectory() as temporary:
    ...
    try:
        ...
        pass
print("op305 successor no-retry V4 hermetic gate fixture passed")
