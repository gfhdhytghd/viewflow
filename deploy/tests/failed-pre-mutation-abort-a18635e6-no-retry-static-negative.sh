...
checker=$root/deploy/check-failed-pre-mutation-abort-a18635e6-no-retry.py
gate=$root/deploy/gate-failed-pre-mutation-abort-a18635e6-no-retry.py
manifest=$root/deploy/failed-pre-mutation-abort-a18635e6-no-retry-manifest.json
launcher=$root/deploy/launch-failed-pre-mutation-abort-a18635e6-no-retry.sh
tmp=$(mktemp -d)
trap 'rm -rf -- "$tmp"' EXIT
