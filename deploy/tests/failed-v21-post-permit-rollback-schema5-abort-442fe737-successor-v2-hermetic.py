#!/usr/bin/env python3
"""Hermetic contract tests for the op442 schema5 abort successor-v2 gate."""
...
GATE_PATH = pathlib.Path(os.environ.get("VF_OP442_GATE",
    ROOT / "deploy/failed-v21-post-permit-rollback-schema5-abort-442fe737-successor-v2-gate.py"))
MANIFEST_PATH = pathlib.Path(os.environ.get("VF_OP442_MANIFEST",
    ROOT / "deploy/failed-v21-post-permit-rollback-schema5-abort-442fe737-successor-v2-manifest.json"))
...
# Offline/live modes require the successor approval to remain absent.  Execute
...
failed = manifest["failed_attempt_v1"]
failed_approval_raw = pathlib.Path(failed["approval"]["path"]).read_bytes()
assert hashlib.sha256(failed_approval_raw).hexdigest() == failed["approval"]["sha256"]
must_fail(lambda: gate.validate_approval(manifest, failed_approval_raw,
    failed["sealed_set"]["manifest"]["sha256"],
    failed["sealed_set"]["gate"]["sha256"],
    failed["sealed_set"]["launcher"]["sha256"]), "failed v1 approval reuse")
...
print("442fe737 schema5 abort successor-v2 hermetic contract tests passed")
