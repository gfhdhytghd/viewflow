...
GATE = Path(os.environ.get("VF_ABORT_GATE",
                           "/home/wilf/data/viewflow/deploy/failed-v21-rollback-abort-845ce422-v2-gate.py"))
MANIFEST = Path(os.environ.get(
    "VF_ABORT_MANIFEST",
    "/home/wilf/data/viewflow/deploy/failed-v21-rollback-abort-845ce422-v2-manifest.json"))
...
def main():
    ...
    check("--local-windows-stop-evidence" not in frozen["argv"],
          "removed coordinator option remained in v2 argv")
    check(all("/failed-v21-rollback-abort-845ce422-v2-" in path
              for path in frozen["outputs"].values()),
          "v2 output namespace is not disjoint from the old set")
    check(frozen["approval_path"].endswith(
        "/failed-v21-rollback-abort-845ce422-v2-execution-approval.json"),
        "v2 approval path is not disjoint from the old approval")
    check(frozen["required_absent"][-2:] == [
        "/home/wilf/.local/state/viewflow/deployments/" + gate.OP
        + "/deployment-release.json",
        "/home/wilf/.local/state/viewflow/deployments/" + gate.OP
        + "/recovery-deployment-publish.json"],
        "immutable old-state sentinel outputs drifted")
    ...
    print("845ce422 abort v2 hermetic tests passed")
