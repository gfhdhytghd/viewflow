...
jq -e --arg op "$op" '
  ...
  keys == ["active_marker","approval_path","argv","coordinator","coordinator_instance_id",
           "execution_authorized","failed_attempt_v1","immutable_inputs","installed_linux","marker_generation",
  ...
  .state == "viewflow-failed-v21-post-permit-rollback-schema5-abort-442fe737-successor-v2-command-manifest" and
  ...
  .failed_attempt_v1 == {
    schema_version:1,
    state:"viewflow-op442-schema5-abort-pre-mutation-approval-gate-failed",
    failure:"execution-approval-remained-in-required-absent",
    mutation_started:false,
    approval:.failed_attempt_v1.approval,
    sealed_set:.failed_attempt_v1.sealed_set} and
  .failed_attempt_v1.approval.sha256 == "04240fee7c849f5d060ca57f87dd3a25b88998e99f966bd0f5603e13a954afcc" and
  (.failed_attempt_v1.approval.path as $old | .required_absent | index($old)) == null and
  (.approval_path as $approval | .required_absent | index($approval)) != null and
...
for token in \
  'execution_approval_may_exist and path == value["approval_path"]' \
  'failed attempt v1 audit evidence differs' \
  'failed["approval"]["path"] in value["required_absent"]' \
  'validate_manifest(manifest, active_required, args.execute or args.resume)'; do
...
while IFS=$'\t' read -r path expected mode size; do
  ...
  (.failed_attempt_v1.approval |
    [.path,.sha256,(.mode|tostring),(.size|tostring)]),
  (.failed_attempt_v1.sealed_set[] |
    [.path,.sha256,(.mode|tostring),(.size|tostring)]) | @tsv' "$manifest")
...
echo '442fe737 schema5 abort successor-v2 sealed-set static checker passed'
