...
approval=$deployment/failed-pre-mutation-abort-305058f7-no-retry-successor1-execution-approval.json
...
manifest='e2131a99e698a01e96a4796144d17d82405f418f2946b6e271815975d865b35c'
gate='604b513283b3c3a0173985bcdf0e42c3c0106f5f962ad225fa5f4fad6f7ad8ff'
launcher='6d24410d784393718e039e8c2da6cb096c1ac0be2814c92927c64985e1615527'
...
approval={'schema_version':4,'state':'viewflow-failed-pre-mutation-no-retry-abort-execution-approved','approved':True,
 ...
 'v4_marker_cli_provenance_sha256':'','candidate_replacement_commit_sha256':H('f'),
 'coordinator_successor_receipt_sha256':H('1'),'coordinator_successor_windows_prestate_sha256':H('2'),
 'successor_authorization_consumed':True,'transaction_implementation':'sealed-fd-native-rust-v4-abort-then-query',
...
provenance={'schema_version':1,'state':'viewflow-no-retry-v4-marker-candidate-provenance','operation_id':op,
 ...
 'base':{},'frozen_source_root':os.path.join(os.path.dirname(v4_cli),'frozen-source'),
 'candidate':{'path':v4_cli,'sha256':v4_cli_sha,'mode':'0700','owner_uid':1000,'owner_gid':1000,
...
terminal={'schema_version':4,'state':'viewflow-failed-pre-mutation-no-retry-vfdqa-abort-terminal','operation_id':op,
 ...
 'windows_old_peer_post_sha256':H('b'),'candidate_manifest_sha256':H('c'),'candidate_tree_sha256':H('d'),
 'candidate_retirement_terminal_sha256':H('e'),'candidate_replacement_commit_sha256':H('f'),
 'coordinator_successor_receipt_sha256':H('1'),'coordinator_successor_windows_prestate_sha256':H('2'),
 'successor_authorization_consumed':True,'marker_absent':True,'abort_claim_absent':True,'release_claim_absent':True,
...
text=text.replace('readonly REVIEWED_MARKER_SHA=e3c981f57a775d343c9e62a7d604a3d4aa3e79f3014e64c9ed8c9e6bbf1581f5','readonly REVIEWED_MARKER_SHA='+candidate_sha)
...
text=text.replace('readonly V4_PROVENANCE_SHA=3d14ca07a7706441b461599d77f504085e490c0ce0405fdb513c022f20114533','readonly V4_PROVENANCE_SHA='+provenance_sha)
deployment=Path(state)/'deployments'/'305058f7deb84c198bad4103d6c4f946'
def sha(path): return hashlib.sha256(Path(path).read_bytes()).hexdigest()
frozen={
 'V4_TERMINAL_SHA': sha(deployment/'no-retry-v4-abort-terminal.json'),
 'V4_EXECUTION_APPROVAL_SHA': sha(deployment/'failed-pre-mutation-abort-305058f7-no-retry-successor1-execution-approval.json'),
 'V4_AUTHORIZATION_SHA': sha(deployment/'no-retry-v4-abort-authorization.json'),
 'V4_ABORT_RECEIPT_SHA': sha(deployment/'no-retry-v4-abort-receipt.json'),
 'V4_ABORT_QUERY_RECEIPT_SHA': sha(deployment/'no-retry-v4-abort-query-receipt.json'),
 'V4_VFDQA_SHA': sha(next(Path(state).glob('.deployment-quarantine.v1.abort-receipt.*.v1'))),
}
for name,value in frozen.items():
 old='readonly '+name+'='+{'V4_TERMINAL_SHA':'9db787bc1a59b5a8b27e7d72e6d16349295a865b1ed1adef9ea9278a557d9384',
  'V4_EXECUTION_APPROVAL_SHA':'6fe9f7d76b8a72b491a4f2898fb700a393e28c8826f90f43fb93eede24495b80',
  'V4_AUTHORIZATION_SHA':'dc7ce0350661902f3278b1e1e58ab487bb92d4953276d6160f33edd58a5ce706',
  'V4_ABORT_RECEIPT_SHA':'1022759a0e265b7097c602b70c094eb3c8b7301c0d5254f9e6dd0fd859a4b230',
  'V4_ABORT_QUERY_RECEIPT_SHA':'785fd373d20154b900a8a8f5b8fc569cb8d3fcf79f001a293cc967f6fe773506',
  'V4_VFDQA_SHA':'092d15a05ed77874d942f44f2697043013a1c5f94350de8610dcb4f8f5b63779'}[name]
 text=text.replace(old,'readonly '+name+'='+value)
