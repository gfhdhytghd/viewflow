...
# Bridge the schema-1 early/no-worker abort terminal into a distinct, fresh
...
if [[ ${BASH_SOURCE[0]} != "$0" ]]; then
    printf 'error: bridge-early-abort-terminal-to-fresh-v21.sh must be executed, not sourced\n' >&2
    return 64
fi
...
post_reattest='' post_reattest_sha='' vfdqa='' vfdqa_sha=''
...
usage() {
    cat <<'EOF'
Usage: bridge-early-abort-terminal-to-fresh-v21.sh (--execute|--resume|--validate-inputs-only) \
  --old-operation-id LOWER32 \
  --early-abort-terminal PATH --early-abort-terminal-sha256 LOWER64 \
  --abort-authorization PATH --abort-authorization-sha256 LOWER64 \
  --abort-receipt PATH --abort-receipt-sha256 LOWER64 \
  --linux-started PATH --linux-started-sha256 LOWER64 \
  --post-abort-reattest PATH --post-abort-reattest-sha256 LOWER64 \
...
only continue that exact plan. The old transient is stopped before the
installed persistent v1.3 service is started; Deskflow is never started.
EOF
}
...
while (($#)); do
    case $1 in
        ...
        --early-abort-terminal) need "$1" "${2-}"; terminal=$2; shift 2 ;;
        --early-abort-terminal-sha256) need "$1" "${2-}"; terminal_sha=$2; shift 2 ;;
        ...
        --linux-started) need "$1" "${2-}"; linux_started=$2; shift 2 ;;
        --linux-started-sha256) need "$1" "${2-}"; linux_started_sha=$2; shift 2 ;;
        --post-abort-reattest) need "$1" "${2-}"; post_reattest=$2; shift 2 ;;
        --post-abort-reattest-sha256) need "$1" "${2-}"; post_reattest_sha=$2; shift 2 ;;
...
main() {
    ...
    validate_inputs
    if [[ $mode == validate-inputs-only ]]; then printf 'schema1 early/no-worker abort inputs validated\n'; return; fi
    ensure_roots; snapshot_inputs; validate_inputs; make_plan; reattest_all_inputs
    ensure_old_retired; reattest_all_inputs
