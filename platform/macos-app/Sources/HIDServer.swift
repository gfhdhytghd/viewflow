...
final class HIDServer: @unchecked Sendable {
    ...
    init(path: String = HIDServer.path,
         receiver: @escaping @Sendable (Int32) -> Int32 = { TrackpadBridge.run("--receive-stdin", inputFD: $0) }) {
    ...
    static var path: String {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/Viewflow/run/hid.sock").path
    }
    ...
    private static func address(_ path: String) throws -> sockaddr_un {
        ...
        guard bytes.count <= MemoryLayout.size(ofValue: address.sun_path) else {
            throw socketError("用户目录路径过长，无法建立 HID 本地连接")
        }
    ...
    private static func socketError(_ message: String) -> NSError {
        NSError(domain: "ViewflowHID", code: 1, userInfo: [NSLocalizedDescriptionKey: message])
    }
    func start() throws {
        ...
        unlink(socketPath)
        guard try Self.withAddress(socketPath, { Darwin.bind(fd, $0, $1) }) == 0 else { throw Self.failure() }
        guard chmod(socketPath, 0o600) == 0, listen(fd, SOMAXCONN) == 0 else { throw Self.failure() }
    ...
    func stop() {
        ...
        if listener >= 0 { shutdown(listener, SHUT_RDWR); listener = -1; unlink(socketPath) }
    ...
    static func relay(path: String = HIDServer.path) -> Int32 {
        do {
            let fd = socket(AF_UNIX, SOCK_STREAM, 0)
            guard fd >= 0 else { throw failure() }
            defer { Darwin.close(fd) }
            guard try withAddress(path, { Darwin.connect(fd, $0, $1) }) == 0 else {
                throw socketError("请先打开 Viewflow.app 并启动原生触控板 HID")
            }
    ...
    static func relay(inputFD: Int32, socketFD: Int32) throws -> Int32 {
            ...
            while true {
                ...
                if events[0].revents & Int16(POLLNVAL) != 0 { throw socketError("HID 输入已关闭") }
                ...
                while sent < count {
                    ...
                    guard n > 0 else { throw socketError("HID 本地连接已结束") }
                    sent += n
                }
            }
            ...
            while received < 4 {
                ...
                guard n > 0 else { throw socketError("HID 接收组件已结束") }
                received += n
            }
