...
// GPU pixel oracle for eliding backdrop only behind decoded opaque patches.
#define wmain viewflow_preview_entry_for_opaque_test
#include "main.cpp"
#undef wmain
#include "sparse_visual_reference.h"
#include <io.h>

static int opaque_pixels() try {
  ...
  std::vector<viewflow::vfgp::AtlasPatch> patches{{0,16,16,16,16,96,96},{0,144,16,144,16,96,96},
      {0,16,144,16,144,96,96},{0,144,144,144,144,96,96}};
  ...
  unsigned phase=0;
  for(float scale:{1.0f,0.75f,0.5f})for(float offset:{0.0f,0.25f})for(unsigned state:{0u,1u,0u,2u}) {
    ++phase;
    ...
    for(unsigned y=0;y<256;++y)for(unsigned x=0;x<256;++x) {
      ...
      for(unsigned c=0;c<3;++c)bytes[i*4+c]=uint8_t(unsigned(rgb[c])*alpha[i]/255);
      bytes[i*4+3]=alpha[i];
    }
    ...
    if(backgrounds!=patches.size()-std::count(flags.begin(),flags.end(),uint8_t(1)))throw std::runtime_error("opaque classification was not applied");
    ...
    std::printf("opaque-pixels phase=%u scale=%.2f offset=%.2f state=%u backgrounds=%zu mismatches=%zu verified=%u\n",phase,scale,offset,state,backgrounds,mismatch,unsigned(verified));
    if(!verified)throw std::runtime_error("opaque backdrop pixel mismatch or stale capture");
  }
  session.Close();pool.Close();return 0;
} catch(hresult_error const& error) {std::fprintf(stderr,"opaque HRESULT=%08x\n",unsigned(error.code().value));return 2;}
catch(std::exception const& error) {std::fprintf(stderr,"%s\n",error.what());return 2;}

int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR,int) {
  FILE* log=nullptr;if(_wfreopen_s(&log,L"opaque-pixels-result.log",L"w",stdout)!=0)return 2;
