...
struct Window {
    ...
    bool configured{},placed{},geometry_pending{};
    int logical_width{},logical_height{},observed_width{},observed_height{},x{},y{},requested_x{},requested_y{},requested_width{},requested_height{};
    std::uint64_t alpha_hash{};Clock::time_point moved_at{};
    ~Window();
...
struct App {
    ...
    std::vector<std::uint8_t> latest_alpha;
    ~App() {
    ...
    void send(std::uint64_t id,vf::InputKind kind,int a=0,int b=0,int c=0,int d=0) {
        auto bytes=vf::pack_input({id,++sequence,kind,a,b,c,d});vf::Writer prefix;prefix.u32(static_cast<std::uint32_t>(bytes.size()));
        write_all(STDOUT_FILENO,prefix.bytes);write_all(STDOUT_FILENO,bytes);
    }
    ...
    static void pointer_enter(void* data,wl_pointer*,std::uint32_t,wl_surface* surface,wl_fixed_t x,wl_fixed_t y) {
        ...
        if(a.pointer_window)a.send(a.pointer_window,vf::InputKind::pointer,static_cast<int>(std::lround(a.pointer_x*a.scale)),static_cast<int>(std::lround(a.pointer_y*a.scale)));
    }
    ...
    static void pointer_button(void* data,wl_pointer*,std::uint32_t,std::uint32_t,std::uint32_t button,std::uint32_t state) {
        auto& a=*static_cast<App*>(data);if(a.pointer_window)a.send(a.pointer_window,vf::InputKind::button,static_cast<int>(button),state==WL_POINTER_BUTTON_STATE_PRESSED);
    }
    ...
    static void keymap(void*,wl_keyboard*,std::uint32_t,int fd,std::uint32_t){close(fd);}
    static void key_enter(void* data,wl_keyboard*,std::uint32_t,wl_surface* surface,wl_array*) {
        auto& a=*static_cast<App*>(data);a.keyboard_window=a.identify(surface);if(a.keyboard_window)a.send(a.keyboard_window,vf::InputKind::focus);
    }
    static void key_leave(void* data,wl_keyboard*,std::uint32_t,wl_surface*) {
        auto& a=*static_cast<App*>(data);a.send(a.keyboard_window,vf::InputKind::release);a.keyboard_window=0;
    }
    static void key(void* data,wl_keyboard*,std::uint32_t,std::uint32_t,std::uint32_t code,std::uint32_t state) {
        auto& a=*static_cast<App*>(data);if(a.keyboard_window)a.send(a.keyboard_window,vf::InputKind::key,static_cast<int>(code),state==WL_KEYBOARD_KEY_STATE_PRESSED);
    }
    static void modifiers(void*,wl_keyboard*,std::uint32_t,std::uint32_t,std::uint32_t,std::uint32_t,std::uint32_t){}
    static void repeat(void*,wl_keyboard*,std::int32_t,std::int32_t){}
...
float a=texture(opacity,p).r;vec3 rgb=vec3(y+1.5748*c.y,y-0.187324*c.x-0.468124*c.y,y+1.8556*c.x);color=vec4(clamp(rgb,vec3(0),vec3(a)),a);})";
    ...
    void region(Window& window) {
        ...
        for(unsigned y=0;y<t.height;++y)for(unsigned x=0;x<t.width;++x){hash^=latest_alpha[std::size_t(t.atlas_y+y)*atlas_width+t.atlas_x+x];hash*=1099511628211ull;}
        if(hash==window.alpha_hash)return;window.alpha_hash=hash;
        ...
        for(unsigned y=0;y<t.height;y+=scale) {
            ...
            for(unsigned x=0;x<=t.width;x+=scale) {
                ...
                if(!opaque && inside){wl_region_add(region,start/scale,y/scale,(x-start+scale-1)/scale,1);inside=false;}
                if(x<t.width && x+scale>t.width){if(inside)wl_region_add(region,start/scale,y/scale,(t.width-start+scale-1)/scale,1);break;}
            }
        }
    ...
    void accept(vf::Frame frame) {
        ...
        for(auto& image:decoded) {
            ...
            decoder->upload(image);atlas_width=current.width;atlas_height=current.height;
            ...
            glTexImage2D(GL_TEXTURE_2D,0,GL_R8,atlas_width,atlas_height,0,GL_RED,GL_UNSIGNED_BYTE,latest_alpha.data());
            ++decoded_count;
            ...
            for(auto it=windows.begin();it!=windows.end();) {
                if(!ids.contains(it->first)){if(pointer_window==it->first)pointer_window=0;if(keyboard_window==it->first){send(it->first,vf::InputKind::release);keyboard_window=0;}it=windows.erase(it);}else ++it;
            }
    ...
    void synchronize_geometry() {
        ...
        for(const auto& client:clients) {
            const auto name=client.value("class",std::string{});if(!name.starts_with("ViewflowReverse-") || !client.value("mapped",false))continue;
            ...
            if(w.placed && (x!=w.x || y!=w.y || width!=w.observed_width || height!=w.observed_height)) {
                // A compositor move/resize is a user edit. Retain the local
                // geometry while its Windows-native counterpart catches up.
                w.geometry_pending=true;w.moved_at=Clock::now();w.requested_x=x;w.requested_y=y;w.requested_width=width;w.requested_height=height;
                send(id,vf::InputKind::geometry,(x-origin_x)*scale,(y-origin_y)*scale,width*scale,height*scale);
            }
            w.x=x;w.y=y;w.logical_width=width;w.logical_height=height;w.observed_width=width;w.observed_height=height;
            if(w.geometry_pending && target_x==w.requested_x && target_y==w.requested_y && std::abs(target_width-w.requested_width)<=1 && std::abs(target_height-w.requested_height)<=1)w.geometry_pending=false;
            if(w.geometry_pending && Clock::now()-w.moved_at<std::chrono::milliseconds(500))continue;
            w.geometry_pending=false;
            if(!w.placed || x!=target_x || y!=target_y || width!=target_width || height!=target_height) {
                ...
                eval("hl.dsp.window.float({action='enable',window="+selector+"})();hl.dsp.window.resize({x="+std::to_string(target_width)+",y="+std::to_string(target_height)+",window="+selector+"})();hl.dsp.window.move({x="+std::to_string(target_x)+",y="+std::to_string(target_y)+",window="+selector+"})()");
                w.x=target_x;w.y=target_y;w.logical_width=target_width;w.logical_height=target_height;w.observed_width=target_width;w.observed_height=target_height;w.placed=true;
            }
        }
    }
    void reader() {
