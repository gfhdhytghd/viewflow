...
int request(lua_State* state, bool stop, bool commitMode = false) {
    try {
        ...
        if (stop) {
            ...
            // An already-reaped stream is stopped too. Make delayed cleanup
            // idempotent after a receiver exits and a new source reuses capacity.
            if (!valid_id(id)) throw std::runtime_error("invalid stream id");
            ...
            // A disconnected or withdrawn capture no longer consumes an
            // active slot. Keep teardown on the compositor thread, outside
            // timer callbacks, before admitting a replacement connection.
            for (auto it = streams.begin(); it != streams.end();) {
                if (!it->second->retired) { ++it; continue; }
                stopStream(it->first, it->second);
                it = streams.erase(it);
            }
