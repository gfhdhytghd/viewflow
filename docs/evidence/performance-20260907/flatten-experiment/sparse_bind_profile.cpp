...
int wmain(int argc, wchar_t**) try {
  ...
  // Alternate both implementations in one process to reduce cold-sample bias.
  for(uint32_t count:{195u,256u})for(bool blurred:{false,true}) {
    std::vector<viewflow::vfgp::AtlasPatch> layout;
    for(uint32_t i=0;i<count;++i)layout.push_back({0,(i%16)*128,(i/16)*128,(i%16)*128,(i/16)*128,128,128});
    CompositionBrush raw=blurred?compositor.CreateBackdropBrush():nullptr;
    auto moved=layout;moved.erase(moved.begin());
    for(size_t i=0;i<moved.size();++i){moved[i].x=uint32_t(i%16)*128;moved[i].y=uint32_t(i/16)*128;}
    std::array<std::vector<int64_t>,2> cold,repack_cost;
    for(unsigned iteration=0;iteration<24;++iteration)for(unsigned slot=0;slot<2;++slot) {
      const bool flat=(iteration+slot)%2;
      const auto start=std::chrono::steady_clock::now();
      auto scene=make_shared_sparse_scene(compositor,a.surface,layout,raw,blurred?12.0f:0.0f,flat);
      const auto elapsed=std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now()-start).count();
      if(iteration>=4)cold[flat].push_back(elapsed);
      const auto repack_start=std::chrono::steady_clock::now();
      update_shared_sparse_visuals(scene,b.surface,moved);
      const auto repack_us=std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now()-repack_start).count();
      if(iteration>=4)repack_cost[flat].push_back(repack_us);
    }
    for(unsigned flat=0;flat<2;++flat) {
      auto& samples=cold[flat];std::sort(samples.begin(),samples.end());
      auto& repacks=repack_cost[flat];std::sort(repacks.begin(),repacks.end());
      std::printf("offscreen-cold flat=%u blur=%u patches=%u samples=%zu median_us=%lld p95_us=%lld repack_median_us=%lld repack_p95_us=%lld no_physical_present=true\n",flat,unsigned(blurred),count,samples.size(),(long long)samples[samples.size()/2],(long long)samples[samples.size()*95/100],(long long)repacks[repacks.size()/2],(long long)repacks[repacks.size()*95/100]);
      std::fflush(stdout);
    }
  }
