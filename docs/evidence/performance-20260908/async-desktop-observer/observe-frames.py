...
for _ in range(40):
 out=ps("$p=@(Get-CimInstance Win32_Process | Where-Object {$_.ExecutablePath -eq '"+root+"\\native-build\\Release\\viewflow_windows_composition_preview.exe'}); if($p.Count -eq 1){Write-Output $p[0].ProcessId}").strip()
...
assert observer_exe in ('viewflow_windows_frame_observer.exe','viewflow_windows_frame_observer_sync.exe')
pid=int(out);name='ViewflowPerf-FrameObserver';log=root+'\\desktop-marker.log'
...
subprocess.run(['scp','-q','-o','BatchMode=yes','wilf@172.16.105.70:'+log.replace(chr(92),'/'),'/tmp/viewflow-integrated-pair/desktop-'+label+'.log'],check=True)
