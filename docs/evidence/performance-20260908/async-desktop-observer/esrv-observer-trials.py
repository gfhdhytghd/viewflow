...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreEsrvObserver';state=Path('/tmp/viewflow-esrv-observer-state.json')
def ps(code):
...
try:
 ...
 paths=['target/release/vf-media-peer','crates/viewflowd/src/atlas_socket_trace.rs','platform/nvenc-encoder/gpu_dmabuf_encoder.cu'];Path('/tmp/viewflow-esrv-observer-trial-source-sha256.json').write_text(json.dumps({p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in paths},indent=2)+'\n')
 for label,priority in [('4k-observer-v2-sync-a1','Normal'),('4k-observer-v2-async-b1','Normal'),('4k-observer-v2-async-b2','Normal'),('4k-observer-v2-sync-a2','Normal')]:
  ...
  desktop=Path('/tmp/viewflow-integrated-pair/desktop-'+label+'.log').read_text(); assert 'exit=0' in desktop and 'desktop-marker ' in desktop,'observer failed'
finally:
 if registered:
  data['restored']=json.loads(ps(restore+"\nGet-Content '"+root+"\\esrv-observer-restored.json' -Raw"));state.write_text(json.dumps(data,indent=2)+'\n');assert data['restored']['priority']==initial['priority'] and data['restored']['thread_priority']==initial['thread_priority']
