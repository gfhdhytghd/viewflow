...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreNvencSplit';state=Path('/tmp/viewflow-nvenc-split-state.json')
def ps(code):
...
Path('/tmp/viewflow-restore-nvenc-splits.ps1').write_text(restore)
...
try:
 ...
 paths=['target/release/vf-media-peer','crates/viewflowd/src/atlas_socket_trace.rs','platform/nvenc-encoder/gpu_dmabuf_encoder.cu','platform/windows-video-compositor/video_compositor.cpp','tools/windows_frame_observer.cpp','tools/windows_window_frame_observer.cpp','tools/windows_observer_timer.h'];Path('/tmp/viewflow-nvenc-split-trial-source-sha256.json').write_text(json.dumps({p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in paths},indent=2)+'\n')
 for label,value in [('4k-nvenc-split-auto-a1','auto'),('4k-nvenc-split-four-b1','4'),('4k-nvenc-split-four-b2','4'),('4k-nvenc-split-auto-a2','auto')]:
