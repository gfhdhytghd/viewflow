...
if(argc!=6)return 2;const char*name=argv[1];int w=atoi(argv[2]),h=atoi(argv[3]);
...
check(av_opt_set(ctx->priv_data,"split_encode_mode",argv[5],0),"split mode");
check(avcodec_open2(ctx,codec,nullptr),"open");int64_t observed_mode=-1;check(av_opt_get_int(ctx->priv_data,"split_encode_mode",0,&observed_mode),"read split mode");printf("split requested=%s option_readback=%lld\n",argv[5],(long long)observed_mode);std::ofstream out(argv[4],std::ios::binary);uint32_t magic=0x56464342,width=w,height=h;out.write((char*)&magic,4);out.write((char*)&width,4);out.write((char*)&height,4);std::vector<double>times;size_t bytes=0;std::ofstream metrics(std::string(argv[4])+".csv");metrics<<"frame,send_receive_us,bytes,pts,flags\n";
for(int i=0;i<330;i++){AVFrame*f=frames[i%frames.size()];f->pts=i;auto start=std::chrono::steady_clock::now();check(avcodec_send_frame(ctx,f),"send");AVPacket*p=av_packet_alloc();check(avcodec_receive_packet(ctx,p),"receive");auto end=std::chrono::steady_clock::now();if(p->pts!=i)throw std::runtime_error("packet identity mismatch");if(i>=30){times.push_back(std::chrono::duration<double,std::micro>(end-start).count());bytes+=p->size;}metrics<<i<<","<<std::chrono::duration<double,std::micro>(end-start).count()<<","<<p->size<<","<<p->pts<<","<<p->flags<<"\n";uint32_t sz=p->size,flags=p->flags;out.write((char*)&sz,4);out.write((char*)&flags,4);out.write((char*)p->data,p->size);av_packet_free(&p);}
