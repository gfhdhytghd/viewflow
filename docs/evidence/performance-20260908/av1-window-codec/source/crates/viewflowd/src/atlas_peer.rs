...
mod receiver {
    ...
    pub async fn run(config: AtlasReceiverConfig, stop: impl Future<Output = ()>) -> Result<()> {
        ...
        let (runtime, _socket_trace) = crate::atlas_socket_trace::runtime("receiver", || {
...
mod tests {
    ...
    #[test]
    fn av1_window_receiver_preserves_negotiated_codec_without_desktop_input() {
        let mut receiver = config();
        receiver.color_codec = AtlasColorCodec::Av1;
        assert!(receiver.desktop.is_none());
        assert!(receiver.pointer.is_none());
        let plan = receiver.plan().unwrap();
        assert_eq!(plan.color.codec, VideoCodec::Av1);
        assert_eq!(receiver.media_policy().plan().unwrap().color, plan.color);
    }
