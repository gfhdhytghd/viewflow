...
fn append_color_codec(command: &mut Command, codec: viewflow_transport::VideoCodec) -> Result<()> {
    match codec {
        viewflow_transport::VideoCodec::H264 => {}
        viewflow_transport::VideoCodec::Av1 => {
            command.args(["--atlas-color-codec", "av1"]);
        }
...
impl AtlasPresenterChild {
    ...
    pub async fn spawn_with_input_capabilities(
        ...
        Self::spawn_with_input_capabilities_codec(
            executable,
            max_frame_bytes,
            proxy_capacity,
            deadline,
    ...
    /// Launch the negotiated color codec with the same native capabilities.
    /// # Errors
    /// Unsupported codecs or native startup failures are reported to the caller.
    pub async fn spawn_with_input_capabilities_codec(
        executable: &Path,
        max_frame_bytes: usize,
        proxy_capacity: usize,
        deadline: Instant,
    ...
    pub async fn spawn_with_input_recovery(
        ...
        Self::spawn_with_input_recovery_codec(
            executable,
            max_frame_bytes,
            proxy_capacity,
            deadline,
    ...
    /// # Errors
    /// Unsupported codecs or native startup failures are reported to the caller.
    pub async fn spawn_with_input_recovery_codec(
        executable: &Path,
        max_frame_bytes: usize,
        proxy_capacity: usize,
        deadline: Instant,
    ...
    async fn spawn_input_mode(
        ...
        if let Some(desktop) = desktop {
            command.env(
                "VIEWFLOW_ATLAS_ICON_DIR",
                crate::window_icon::receiver_directory(),
            );
            command
        ...
        append_color_codec(&mut command, color_codec)?;
        #[cfg(windows)]
    ...
    pub async fn spawn(executable: &Path, deadline: Instant) -> Result<Self> {
        let mut command = Command::new(executable);
        command.arg("--stdin-atlas-v5").arg("--atlas-sparse-v1");
        #[cfg(windows)]
        command.creation_flags(windows_sys::Win32::System::Threading::CREATE_NO_WINDOW);
    ...
    pub async fn spawn_with_capacity(
        ...
        Self::spawn_with_capacity_codec(
            executable,
            max_frame_bytes,
            proxy_capacity,
            deadline,
    ...
    /// # Errors
    /// Unsupported codecs or native startup failures are reported to the caller.
    pub async fn spawn_with_capacity_codec(
        executable: &Path,
        max_frame_bytes: usize,
        proxy_capacity: usize,
        deadline: Instant,
    ...
    pub async fn spawn_with_dispositions(
        ...
        Self::spawn_with_dispositions_codec(
            executable,
            max_frame_bytes,
            proxy_capacity,
            deadline,
    ...
    /// # Errors
    /// Unsupported codecs or native startup failures are reported to the caller.
    pub async fn spawn_with_dispositions_codec(
        executable: &Path,
        max_frame_bytes: usize,
        proxy_capacity: usize,
        deadline: Instant,
...
mod tests {
    ...
    #[tokio::test]
    async fn negotiated_codec_reaches_every_window_launch_mode() {
        use std::os::unix::fs::PermissionsExt;
        use viewflow_transport::VideoCodec;
        for codec in [VideoCodec::H264, VideoCodec::Av1] {
            for mode in 0..6 {
                let root = tempfile::tempdir().unwrap();
                let executable = root.path().join("presenter");
                let ready = match mode {
                    0 => "atlas-native-ready input_enabled=false",
                    1 => "atlas-native-ready disposition=v1 input_enabled=false",
                    2 => "atlas-native-ready disposition=v1 input_enabled=true pointer=v1",
                    3 => "atlas-native-ready disposition=v1 input_enabled=true pointer=v1 wheel=v1",
                    4 => {
                ...
                let mut child = match mode {
                    0 => AtlasPresenterChild::spawn_with_capacity_codec(
                        &executable,
                        4096,
                        2,
                        deadline,
                    ...
                    .await
                    .unwrap(),
                    1 => AtlasPresenterChild::spawn_with_dispositions_codec(
                        &executable,
                        4096,
                        2,
                        deadline,
                    ...
                    .unwrap(),
                    2..=4 => {
                        AtlasPresenterChild::spawn_with_input_capabilities_codec(
                            &executable,
                            4096,
                            2,
                            deadline,
                    ...
                    _ => {
                        AtlasPresenterChild::spawn_with_input_recovery_codec(
                            &executable,
                            4096,
                            2,
                            deadline,
                ...
                let arguments =
                    std::fs::read_to_string(root.path().join("presenter.args")).unwrap();
                let mut expected = vec![
                    "--stdin-atlas-v5",
                    "--atlas-sparse-v1",
                    "--max-frame-bytes",
                    "4096",
                    "--atlas-proxy-capacity",
                    "2",
                ];
                if mode >= 1 {
                    expected.push("--atlas-disposition-v1");
                }
                if mode >= 2 {
                    expected.push("--atlas-pointer-v1");
                }
                if mode >= 3 {
                    expected.push("--atlas-wheel-v1");
                }
                if mode >= 4 {
                    expected.push("--atlas-keyboard-v1");
                }
                if mode >= 5 {
                    expected.push("--atlas-input-recovery-v1");
                }
                if codec == VideoCodec::Av1 {
                    expected.extend(["--atlas-color-codec", "av1"]);
                }
