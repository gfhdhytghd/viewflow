...
int main(int argc,char** argv) {
  ...
  for(unsigned mode:{1u,2u}) {
    ...
    // Unchanged AV1 sparse frames must preserve every alpha byte even when the
    // GPU comparison suppresses the full readback on subsequent submissions.
    for (unsigned repeat = 0; repeat < 2; ++repeat) {
      ++atlas.frame_id;
      ++atlas.capture_timestamp_ns;
      output = nullptr;
      require(vf_gpu_dmabuf_encoder_encode_sparse_recoverable(
                  encoder, &atlas, &scene, 1, stamp + 10000000000LL, &output) == 0 && output,
              "unchanged sparse encode");
      std::vector<unsigned char> repeatedAlpha(alpha.size());
      require(vf_gpu_dmabuf_output_copy_raw_alpha(output, repeatedAlpha.data(),
                  repeatedAlpha.size(), &required) == 0 && repeatedAlpha == alpha,
              "unchanged sparse alpha remains byte exact");
      require(vf_gpu_dmabuf_output_destroy(output) == 0, "unchanged sparse cleanup");
    }
