...
label=sys.argv[1] if len(sys.argv)>1 else '4k-deep-socket';p=Path('/tmp/viewflow-integrated-pair')
...
for row in sorted(valid,key=lambda x:x['feedback_us'],reverse=True)[:8]:
 ...
 for role,data in traces.items():
  ...
  states={f'{op}/{state}':sum(x['op']==op and x['status']==state for x in selected) for op in ['send','recv','writable','recv_ready','recv_syscall','recv_wake'] for state in ['ok','pending','pending_budget','would_block','error','wake']}
  ...
  for j,x in enumerate(selected):
   if x['op']!='recv_ready' or not x['status'].startswith('pending'):continue
   previous_calls=[y for y in selected[:j] if y['op']=='recv_syscall']
   wakes=[y for y in selected[j+1:] if y['op']=='recv_wake' and y['before_ns']>=x['before_ns']]
   polls=[y for y in selected[j+1:] if y['op']=='recv_ready' and y['before_ns']>=x['after_ns']]
   first_wake=wakes[0] if wakes else None;next_poll=polls[0] if polls else None
   pending.append(dict(time_ms=x['relative_ms'],kind=x['status'],previous_syscall_status=previous_calls[-1]['status'] if previous_calls else None,
       wake_ms=first_wake['relative_ms'] if first_wake else None,next_poll_ms=next_poll['relative_ms'] if next_poll else None))
  out['roles'][role]['pending_intervals']=pending
...
result['socket_calls']={role:{op:{'durations_us':stats([(x['after_ns']-x['before_ns'])/1000 for x in data if x['op']==op]),'states':{state:sum(x['op']==op and x['status']==state for x in data) for state in ['ok','pending','pending_budget','would_block','error','wake']}} for op in ['send','recv','writable','recv_ready','recv_syscall','recv_wake']} for role,data in traces.items()}
