...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();name=sys.argv[1] if len(sys.argv)>1 else 'probe-wpr-network';port=sys.argv[2] if len(sys.argv)>2 else '49101'
assert all(c.isalnum() or c=='-' for c in name) and port in ['49073','49101']
if len(sys.argv)<=1:subprocess.run(['scp','-q','-r','-o','BatchMode=yes','/tmp/viewflow-network-reader-publish','wilf@172.16.105.70:'+root.replace('\\','/')+'/'],check=True)
s="Set-Location '"+root+"'; & .\\viewflow-network-reader-publish\\reader.exe '"+name+".etl' '"+name+".jsonl' '"+port+"'; exit $LASTEXITCODE"
