...
out=Path('docs/evidence/performance-20260908/deep-socket/network-trace');out.mkdir(exist_ok=True)
label='4k-deep-network';trial=out/label;trial.mkdir(exist_ok=True);raw=Path('/tmp/viewflow-integrated-pair');texts={}
...
for source,dest in [('/tmp/viewflow-4k-deep-network-socket-summary.json','socket-summary.json'),('/tmp/viewflow-analyze-deep-socket.py','analyze-deep-socket.py'),('/tmp/viewflow-owned-output-deep-network-state.json','owned-output-state.json')]:shutil.copy(source,out/dest)
for name in ['deep-network-analysis','deep-network-correlation','owned-output-deep-network']:
...
for p in ['deep-network-trial.py','owned-output-deep-network.py']: shutil.copy('/tmp/viewflow-'+p,out/p)
print(json.dumps(summary,indent=2))

for n in ['deep-network.jsonl','deep-network.jsonl.summary.json','deep-network-kernel-correlation.json','correlate-kernel-socket.py','deep-network-trial-source-sha256.json']:
 shutil.copy('/tmp/viewflow-'+n,out/n)
ledger=json.loads((out/'deep-network-trial-source-sha256.json').read_text())
assert all(hashlib.sha256(Path(p).read_bytes()).hexdigest()==v for p,v in ledger.items())
