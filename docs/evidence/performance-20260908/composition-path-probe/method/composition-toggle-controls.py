...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreCompositionToggleProbe';state=Path('/tmp/viewflow-composition-toggle-state.json')
def ps(code):
...
Path('/tmp/viewflow-restore-composition-toggle.ps1').write_text(restore)
...
try:
 ...
 for label,mode in [('probe-toggle-ab','toggle'),('probe-toggle-ba','toggle-reverse')]:
  ...
  with open('/tmp/viewflow-composition-latency-probe/'+label+'-control.log','w') as log:
   r=subprocess.run(['python3','/tmp/viewflow-run-composition-toggle-probe.py',label,mode],stdout=log,stderr=subprocess.STDOUT)
