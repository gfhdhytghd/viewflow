...
for label in ['probe-toggle-ab','probe-toggle-ba']:
 ...
 for x in rows:
  ...
  assert x['backgrounds']==(4 if (x['phase']%2==0)!=(header['mode']=='toggle-reverse') else 0)
 ...
 for phase in sorted({p['phase'] for p in pairs}):
  ...
  phases.append({'phase':phase,'backgrounds':selected[0]['backgrounds'],'first_frame':selected[0]['frame'],'last_frame':selected[-1]['frame'],**stat(selected)})
 # Aggregated latency is valid; do not aggregate change rate across noncontiguous phases.
 grouped={}
 for bg in [0,4]:
  grouped[str(bg)]=stat([p for p in pairs if p['backgrounds']==bg and not p['warmup']]);del grouped[str(bg)]['observed_change_hz']
...
(root/'toggle-comparison.json').write_text(json.dumps(results,indent=2)+'\n')
