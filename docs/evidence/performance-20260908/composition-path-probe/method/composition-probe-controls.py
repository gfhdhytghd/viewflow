...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreCompositionProbe';state=Path('/tmp/viewflow-composition-probe-state.json')
def ps(code):
...
Path('/tmp/viewflow-restore-composition-probe.ps1').write_text(restore)
...
try:
 ...
 for label,mode in [('probe-winrt-a1','winrt'),('probe-hostflag-b1','hostflag'),('probe-sparse-c1','sparse'),('probe-elided-d1','elided'),('probe-elided-d2','elided'),('probe-sparse-c2','sparse'),('probe-hostflag-b2','hostflag'),('probe-winrt-a2','winrt')]:
  ...
  with open('/tmp/viewflow-composition-latency-probe/'+label+'-control.log','w') as log:
   r=subprocess.run(['python3','/tmp/viewflow-run-composition-latency-probe.py',label,mode],stdout=log,stderr=subprocess.STDOUT)
