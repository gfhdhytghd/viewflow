...
files=['tools/windows_composition_latency_probe.cpp','tools/windows_observer_timer.h','platform/windows-composition-preview/timer_resolution_guard.h','platform/windows-composition-preview/atlas_record.h','platform/windows-composition-preview/sparse_shared_visuals.h']
Path('/tmp/viewflow-composition-latency-probe-source-sha256.json').write_text(json.dumps({n:hashlib.sha256(Path(n).read_bytes()).hexdigest() for n in files},indent=2)+'\n')
...
exit /b %errorlevel%
'''
p=Path('/tmp/viewflow-composition-latency-probe-build.cmd');p.write_bytes(cmd.replace('\n','\r\n').encode())
subprocess.run(['scp','-q','-o','BatchMode=yes',str(p),'wilf@172.16.105.70:'+root.replace(chr(92),'/')+'/composition-latency-probe-build.cmd'],check=True)
s="$ErrorActionPreference='Stop';Set-Location '"+root+"'; if(@(Get-CimInstance Win32_Process | Where-Object {$_.ExecutablePath -eq (Get-Location).Path+'\\viewflow_windows_composition_latency_probe.exe'}).Count){throw 'probe running'}; cmd.exe /d /c 'composition-latency-probe-build.cmd > composition-latency-probe-build.log 2>&1';$code=$LASTEXITCODE;Get-Content composition-latency-probe-build.log;exit $code"
r=subprocess.run(['ssh','-o','BatchMode=yes','wilf@172.16.105.70','powershell','-NoProfile','-EncodedCommand',base64.b64encode(s.encode('utf-16le')).decode()],capture_output=True,timeout=240);Path('/tmp/viewflow-composition-latency-probe-build.log').write_bytes(r.stdout+r.stderr);print(r.returncode,r.stdout.decode(errors='replace'));r.check_returncode()
