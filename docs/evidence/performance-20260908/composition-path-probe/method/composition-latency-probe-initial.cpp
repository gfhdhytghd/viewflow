...
struct Wuc {
  ...
  Wuc(HWND window,Renderer& renderer){
  ...
  void submit(Renderer& renderer,Timing& t){
    ...
    t.mutation=qpc();visual.Brush(brushes[index]);t.committed=qpc();
  }
};
struct Dcomp {
...
int run(bool direct,UINT duration,FILE* log){
  ...
  if(direct)dcomp=std::make_unique<Dcomp>(window.value,renderer);else wuc=std::make_unique<Wuc>(window.value,renderer);
...
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR,int){
  ...
  const bool valid=(mode==L"winrt"||mode==L"dcomp")&&end&&!*end&&duration>=1000&&duration<=60000;LocalFree(args);if(!valid)return 2;
  FILE* log{};if(_wfopen_s(&log,path.c_str(),L"w")||!log)return 3;
  int result=1;
  try{ObserverTimer timer(true,log);result=run(mode==L"dcomp",UINT(duration),log);}
