...
struct Timing {
  uint32_t frame{};
  uint32_t phase{}, phase_frame{}, backgrounds{};
...
struct Wuc {
  ...
  Wuc(HWND window,Renderer& renderer,const std::wstring& mode){
    ...
    toggle=mode==L"toggle"||mode==L"toggle-reverse";
    reverse=mode==L"toggle-reverse";
  ...
  void submit(Renderer& renderer,Timing& t){
    ...
    if(toggle){
      ...
      t.backgrounds=((t.phase%2==0)!=reverse)?4u:0u;
      if(t.phase_frame==0)for(auto& [key,node]:scene->nodes)
...
int run(const std::wstring& mode,UINT duration,FILE* log){
  ...
  const bool toggle=mode==L"toggle"||mode==L"toggle-reverse";
  ...
  if(toggle)std::fprintf(log,"probe-toggle phase_frames=240 warmup_frames=30 nodes_retained=1 reverse=%u\n",unsigned(mode==L"toggle-reverse"));
  ...
  for(const auto& t:timings)std::fprintf(log,"probe-frame frame=%u phase=%u phase_frame=%u backgrounds=%u render_qpc=%lld drawn_qpc=%lld begin_qpc=%lld copied_qpc=%lld ended_qpc=%lld mutation_qpc=%lld committed_qpc=%lld\n",t.frame,t.phase,t.phase_frame,t.backgrounds,t.render,t.drawn,t.begin,t.copied,t.ended,t.mutation,t.committed);
...
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR,int){
  ...
  const bool valid=(mode==L"winrt"||mode==L"dcomp"||mode==L"hostflag"||mode==L"sparse"||mode==L"elided"||mode==L"toggle"||mode==L"toggle-reverse")&&end&&!*end&&duration>=1000&&duration<=60000;LocalFree(args);if(!valid)return 2;
