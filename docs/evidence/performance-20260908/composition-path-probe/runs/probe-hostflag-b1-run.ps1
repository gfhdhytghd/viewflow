...
$r='C:\Users\wilf\Viewflow\perf-isolated-8e97c1b751fc';$label='probe-hostflag-b1';$mode='hostflag';$probe='ViewflowPerf-CompositionProbe';$observer='ViewflowPerf-CompositionProbeObserver';$exe=$r+'\viewflow_windows_composition_latency_probe.exe';$dd=$r+'\native-build\Release\viewflow_windows_frame_observer.exe';$pidSeen=0;$registeredProbe=$false;$registeredObserver=$false;$probeResult=-1;$observerResult=-1;
...
try {
 $args=$mode+' 25000 "'+$r+'\'+$label+'-probe.log"';
 ...
 $args=$pidSeen.ToString()+' 20000 "'+$r+'\'+$label+'-desktop.log" physical4k timer1';
