...
Path('/tmp/viewflow-commit-cadence-final-state.json').write_text(json.dumps(v,indent=2)+'\n');print('original Windows binaries/PDB/configs and cleanup verified; prototype source archived and production source restored; Linux binary unchanged',v['process_id'],v['priority'],len(v['threads']))
