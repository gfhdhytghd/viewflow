...
with log.open('w') as output:
 ...
 try:
  ...
  c=matching[0]
  p=json.loads(subprocess.check_output(['target/release/vf-media-peer','probe','--compositor-pid',os.environ['VIEWFLOW_NESTED_COMPOSITOR_PID'],'--window',c['address']]))
  if c['size']!=[1920,1200] or not (3840<=p['width']<=3968 and 2400<=p['height']<=2432):raise RuntimeError('wrong fixture resolution')
  s=json.loads((root/'send.json').read_text());s['compositor_pid']=int(os.environ['VIEWFLOW_NESTED_COMPOSITOR_PID']);s['windows'][0].update(address=c['address'],width=p['width'],height=p['height'],geometry_epoch=p['geometry_epoch']);(root/'send.json').write_text(json.dumps(s,indent=2)+'\n')
  (root/('fixture-'+label+'.json')).write_text(json.dumps(p,indent=2)+'\n');print('fixture',p,flush=True)
  subprocess.run(['python3','/tmp/viewflow-integrated-all-trial.py',label,'new'],check=True)
 finally:
