...
def request(op,data):
 ...
 result=ctl('repl','return hl.plugin.viewflow_capture_commit_test.window_stream_'+op+'('+json.dumps(str(path))+')')
...
try:
 ...
 for i in range(2):
  ...
  p=subprocess.Popen(['/tmp/viewflow-frame-fixture-build/viewflow_linux_frame_fixture',state['screen'],'90000'],env=env,stdout=log,stderr=subprocess.STDOUT);fixtures.append(p)
  for _ in range(60):
 ...
 try:
  lib=ctypes.CDLL('/tmp/viewflow-nested.UgMEB2/readback.so');lib.save_frame.argtypes=[ctypes.c_int]+[ctypes.c_uint]*4+[ctypes.c_uint64,ctypes.c_uint,ctypes.c_char_p]
  w,h,fourcc,stride,modifier,offset=struct.unpack_from('>IIIIQQ',data,64)
  assert lib.save_frame(fds[0],w,h,stride,fourcc,modifier,offset,str(root/'first-frame.ppm').encode())==0
