...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreCommitCadence';state=Path('/tmp/viewflow-commit-cadence-state.json')
def ps(code):
...
Path('/tmp/viewflow-restore-commit-cadence.ps1').write_text(restore)
...
try:
 ...
 paths=['crates/viewflowd/Cargo.toml','crates/viewflowd/src/atlas_source.rs','crates/viewflowd/src/gpu_atlas_capture.rs','crates/viewflowd/src/gpu_atlas_device.rs','crates/viewflowd/src/gpu_atlas_session.rs','crates/viewflowd/src/hyprcapture_gpu_socket.rs','platform/windows-composition-preview/main.cpp','platform/windows-video-compositor/gpu_timestamp_probe.h','target/release/vf-media-peer','tools/windows_frame_observer.cpp','tools/windows_observer_timer.h','platform/viewflow-capture/src/main.cpp','platform/viewflow-capture/src/capture_commit_schedule.hpp','platform/viewflow-capture/src/capture_cadence.hpp','platform/viewflow-capture/CMakeLists.txt','/tmp/viewflow-commit-cadence-build/viewflow-capture-commit-test.so'];Path('/tmp/viewflow-commit-cadence-trial-source-sha256.json').write_text(json.dumps({p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in paths},indent=2)+'\n')
 for label,codec,value,capture_mode in [('4k-commit-grid-minimal-a1','h264','minimal','grid'),('4k-commit-event-minimal-b1','h264','minimal','commit'),('4k-commit-event-minimal-b2','h264','minimal','commit'),('4k-commit-grid-minimal-a2','h264','minimal','grid')]:
  capture_events='1'
  send=json.loads(send_path.read_text());receive=json.loads(original_receive);send['media']['color_codec']=codec;receive['color_codec']=codec;receive['native_presenter']=root+'\\native-trace-controls-build\\Release\\viewflow_windows_composition_preview.exe'
  ...
  phase={'label':label,'codec':codec,'diagnostic_mode':value,'capture_events':capture_events,'capture_mode':capture_mode,'before':before};data['phases'].append(phase);state.write_text(json.dumps(data,indent=2)+'\n');print('BEGIN',label,before,flush=True)
  env=dict(os.environ,VIEWFLOW_COMMIT_MODE=capture_mode,VIEWFLOW_CAPTURE_EVENTS=capture_events,VIEWFLOW_OBSERVE_FRAMES='1',VIEWFLOW_OBSERVE_PHYSICAL4K='1',VIEWFLOW_OBSERVER_EXE='viewflow_windows_frame_observer.exe',VIEWFLOW_OBSERVER_TIMER='1',VIEWFLOW_TRIAL_TRACE_MODE=value,VIEWFLOW_OBSERVER_NATIVE_DIR='native-trace-controls-build',VIEWFLOW_ALPHA_COPY_PROFILE=('0' if value=='minimal' else '1'),VIEWFLOW_ALPHA_OUTPUT_COPY='0',VIEWFLOW_TRIAL_GPU_TIMINGS=('0' if value=='minimal' else 'all'),VIEWFLOW_GPU_TILE_COPY='0',VIEWFLOW_QUIC_POLL='0',VIEWFLOW_QUIC_SOCKET_TRACE=('0' if value=='minimal' else '1'));env.pop('VIEWFLOW_QUIC_BURST_PACKETS',None)
  with open('/tmp/'+label+'.log','w') as log:r=subprocess.run(['python3','/tmp/viewflow-commit-full-resolution-trial.py',label],env=env,stdout=log,stderr=subprocess.STDOUT)
