...
source=Path('/tmp/viewflow-integrated-pair/source-4k-udp-stack-normal.log').read_text();receiver=Path('/tmp/viewflow-integrated-pair/receiver-4k-udp-stack-normal.log').read_text()
stackdata=[json.loads(l) for l in Path('/tmp/viewflow-udp-stack-normal.jsonl').read_text().splitlines()];delivery=sorted([x for x in stackdata if 'delivery'in x],key=lambda x:x['delivery']['qpc']);kernel=sorted([x for x in stackdata if x.get('name')=='UdpIp/Recv'],key=lambda x:x['qpc'])
meta=json.loads(Path('/tmp/viewflow-udp-stack-normal.jsonl.summary.json').read_text());assert meta['events_lost']==0 and meta['missing']==0 and meta['selected']==len(delivery)==len(kernel)
...
Path('/tmp/viewflow-udp-stack-normal-correlation.json').write_text(json.dumps(result,indent=2)+'\n');print('PATHS',result['paths'],'DELTA',result['delivery_to_kernel_ms'])
