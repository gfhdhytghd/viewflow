...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreEsrvThread';state=Path('/tmp/viewflow-esrv-thread-state.json')
def ps(code):
...
try:
 ...
 paths=['target/release/vf-media-peer','crates/viewflowd/src/atlas_socket_trace.rs','platform/nvenc-encoder/gpu_dmabuf_encoder.cu'];Path('/tmp/viewflow-esrv-thread-trial-source-sha256.json').write_text(json.dumps({p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in paths},indent=2)+'\n')
 for label,priority in [('4k-esrv-thread-critical-a1','TimeCritical'),('4k-esrv-thread-normal-b1','Normal'),('4k-esrv-thread-normal-b2','Normal'),('4k-esrv-thread-critical-a2','TimeCritical')]:
