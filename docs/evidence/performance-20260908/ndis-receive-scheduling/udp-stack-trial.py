...
paths=['target/release/vf-media-peer','crates/viewflowd/src/atlas_socket_trace.rs','platform/nvenc-encoder/gpu_dmabuf_encoder.cu'];Path('/tmp/viewflow-udp-stack-trial-source-sha256.json').write_text(json.dumps({p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in paths},indent=2)+'\n')
started=False
try:
 ...
 subprocess.run(['python3','/tmp/viewflow-full-resolution-trial.py','4k-udp-stack'],env=env,check=True)
finally:
 if started:ps("wpr -stop '"+root+"\\udp-stack.etl'; exit $LASTEXITCODE")
ps('wpr -status; exit 0')
subprocess.run(['python3','/tmp/viewflow-run-udp-stack-reader.py'],check=True)
