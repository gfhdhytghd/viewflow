...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreWindowTimers';state=Path('/tmp/viewflow-window-timer-state.json')
def ps(code):
...
Path('/tmp/viewflow-restore-window-timers.ps1').write_text(restore)
...
try:
 ...
 paths=['target/release/vf-media-peer','crates/viewflowd/src/atlas_socket_trace.rs','platform/nvenc-encoder/gpu_dmabuf_encoder.cu','platform/windows-video-compositor/video_compositor.cpp','tools/windows_frame_observer.cpp','tools/windows_window_frame_observer.cpp','tools/windows_observer_timer.h'];Path('/tmp/viewflow-window-timer-trial-source-sha256.json').write_text(json.dumps({p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in paths},indent=2)+'\n')
 for label,value in [('4k-window-timer0-a1','0'),('4k-window-timer1-b1','1'),('4k-window-timer1-b2','1'),('4k-window-timer0-a2','0')]:
  ...
  window=Path('/tmp/viewflow-integrated-pair/window-'+label+'.log').read_text();assert 'exit=0' in window and 'window-marker ' in window
finally:
 if registered:
  ps(restore)
