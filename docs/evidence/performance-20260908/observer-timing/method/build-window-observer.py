...
for src in ['platform/windows-composition-preview/CMakeLists.txt','tools/windows_window_frame_observer.cpp']:
 subprocess.run(['scp','-q','-o','BatchMode=yes',src,'wilf@172.16.105.70:'+root.replace(chr(92),'/')+'/'+src],check=True)
s="$ErrorActionPreference='Stop'; Set-Location '"+root+"'; $cmake='C:\\Program Files (x86)\\Microsoft Visual Studio\\2022\\BuildTools\\Common7\\IDE\\CommonExtensions\\Microsoft\\CMake\\CMake\\bin\\cmake.exe'; & $cmake --build native-build --config Release --target viewflow_windows_window_frame_observer --parallel 2 > window-observer-build.log 2>&1; $code=$LASTEXITCODE; Get-Content window-observer-build.log -Tail 18; if($code){exit $code}; exit $LASTEXITCODE"
