...
p=Path('/home/wilf/data/viewflow/docs/evidence/performance-20260908/native-alpha-reuse');p.mkdir(exist_ok=True)
raw=Path('/tmp/viewflow-integrated-pair');out=[]
for label in sys.argv[1:]:
 mode='expanded' if '-expand-' in label else 'reused';d=p/label;d.mkdir(exist_ok=True)
 ...
 # Source is held on native-owned-view in both Windows variants.
 ...
 native_rows=[timeline.numbers(x) for x in timeline.rows(texts['receiver'],'atlas-native-alpha ')]
 host_rows=[timeline.numbers(x) for x in timeline.rows(texts['receiver'],'atlas-native-host-us ') if int(x['frame'])>=30]
 committed=[timeline.numbers(x) for x in timeline.rows(texts['receiver'],'atlas-native-timing ') if x.get('phase')=='committed' and int(x['frame'])>=30]
 row['native_alpha_records']=len(native_rows)
 row['native_alpha_reused']=sum(x['reused'] for x in native_rows)
 row['alpha_host_ms']=timeline.stats([x['alpha']/1000 for x in host_rows])
 row['native_mode']=[l for l in texts['receiver'].splitlines() if l.startswith('atlas-alpha-storage ')]
 row['native_commit_rate']=(len(committed)-1)*committed[0]['frequency']/(committed[-1]['qpc']-committed[0]['qpc']) if len(committed)>1 else None
 out.append(row)
 print('native',row['native_mode'],'reuse',row['native_alpha_reused'],row['native_alpha_records'],'alpha_host',round(row['alpha_host_ms']['median'],4),'commit_rate',round(row['native_commit_rate'],3))
