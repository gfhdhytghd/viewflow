...
for label in sys.argv[1:]:
 ...
 row['inplace_diagnostics']=[t.numbers(x) for x in t.rows(texts['receiver'],'atlas-inplace-surface ')]
 assert bool(row['inplace_diagnostics'])==('-inplace1-' in label)
 row['inplace_applied_samples']=sum(x['applied'] for x in row['inplace_diagnostics'])
 row['copy_ready_stage_caveat']='When in-place is applied, copy-ready marks prepared scene metadata, preceding the deferred bound copy. Do not use this stage as GPU copy completion.'
 result.append(row)
...
Path('/tmp/viewflow-inplace-comparison.json').write_text(json.dumps(result,indent=2)+'\n')
