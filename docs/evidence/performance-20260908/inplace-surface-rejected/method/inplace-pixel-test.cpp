...
static int coalesce_pixels() try {
  ...
  unsigned phase=0,bound_updates=0;
  GpuSurfaceCandidate persistent;
  for(float scale:{1.0f,0.75f,0.5f})for(float offset:{0.0f,0.25f})for(unsigned state:{0u,0u,1u,1u,0u,0u,2u,2u,3u,3u,4u,4u,5u,5u}) {
    ++phase;
    ...
    if(!persistent.surface)persistent=make_gpu_surface(owner,compositor,frame.width,frame.height);
    ...
    if(candidate_next.reuse) {
      copy_gpu_frame(owner,persistent.surface,frame);++bound_updates;
    } else {
      persistent=stage_gpu_surface(owner,compositor,frame);
      copy_gpu_frame(owner,persistent.surface,frame);
  ...
  std::printf("bound_updates=%u phases=%u\n",bound_updates,phase);
  if(bound_updates<42)throw std::runtime_error("bound updates were not exercised");
