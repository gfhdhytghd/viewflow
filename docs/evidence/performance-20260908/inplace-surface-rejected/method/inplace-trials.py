...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreInplace';state=Path('/tmp/viewflow-inplace-state.json')
def ps(code):
...
Path('/tmp/viewflow-restore-inplaces.ps1').write_text(restore)
...
try:
 ...
 paths=['target/release/vf-media-peer','crates/viewflowd/src/atlas_socket_trace.rs','platform/nvenc-encoder/gpu_dmabuf_encoder.cu','platform/windows-video-compositor/video_compositor.cpp','tools/windows_frame_observer.cpp','tools/windows_window_frame_observer.cpp','tools/windows_observer_timer.h','platform/windows-composition-preview/main.cpp','platform/windows-composition-preview/stable_surface_layout.h'];Path('/tmp/viewflow-inplace-trial-source-sha256.json').write_text(json.dumps({p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in paths},indent=2)+'\n')
 for label,value in [('4k-inplace0-a1','0'),('4k-inplace1-b1','1'),('4k-inplace1-b2','1'),('4k-inplace0-a2','0')]:
  ...
  applied=[line for line in receiver.splitlines() if line.startswith('atlas-inplace-surface ') and 'applied=1' in line]
  assert (len(applied)>10) if value=='1' else not applied
finally:
 if registered:
  ps(restore)
