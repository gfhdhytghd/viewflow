...
label,mode,port,ecn=sys.argv[1:];port=int(port);ecn=int(ecn);assert port in [49101,49102] and ecn in [0,2];assert mode in ['blocking','nonblocking'] and all(c.isalnum() or c=='-' for c in label)
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();directory=root+'\\interactive-udp-'+label;task='ViewflowPerf-PairedUDPProbe-'+str(port);nonce=secrets.randbits(64) or 1;counts=[20]*4+[150,20]*24
...
try:
 ...
 sock=socket.socket(socket.AF_INET,socket.SOCK_DGRAM);sock.bind(('172.16.105.62',0));sock.setsockopt(socket.IPPROTO_IP,socket.IP_TOS,ecn);sock.setsockopt(socket.IPPROTO_IP,10,2);sock.settimeout(2)
 try:
  for batch,count in enumerate(counts):
   payload=secrets.token_bytes(1419);start=time.monotonic_ns()
   for sequence in range(count):
    packet=struct.pack('<IIIIQ',0x56465542,batch,sequence,count,nonce)+payload;assert sock.sendto(packet,('172.16.105.70',port))==1443
   queued=time.monotonic_ns();ack,peer=sock.recvfrom(1500);end=time.monotonic_ns();assert peer==('172.16.105.70',port) and struct.unpack('<IIIIQ',ack)==(0x5646414B,batch,count,count,nonce)
   result.append(dict(batch=batch,packets=count,source_begin_ns=start,source_end_ns=end,send_us=(queued-start)/1000,round_trip_us=(end-start)/1000));time.sleep(.2)
 finally:sock.close()
finally:
 (out/'source.json').write_text(json.dumps({'mode':mode,'bytes':1443,'ecn':ecn,'df':True,'payload':'nonce header and random tail','batches':result},indent=2)+'\n')
 if created:
