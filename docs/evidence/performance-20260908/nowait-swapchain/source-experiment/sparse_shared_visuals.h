...
struct SharedSparseScene {
  using Key = std::tuple<uint32_t,uint32_t,uint32_t,uint32_t,bool>;
  struct Node {
...
inline SharedSparsePlan stage_shared_sparse_visuals(SharedSparseScene const& scene,
  ...
  std::set<std::tuple<uint32_t,uint32_t,uint32_t,uint32_t>> rectangles;
  for(const auto& patch:patches) {
    const bool hidden_backdrop=scene.backdrop && !opaque.empty() && opaque[patch_index];
    ++patch_index;
    if(!rectangles.emplace(patch.source_x,patch.source_y,patch.width,patch.height).second)
      throw std::runtime_error("duplicate sparse source rectangle");
    SharedSparseScene::Key key{patch.source_x,patch.source_y,patch.width,patch.height,hidden_backdrop};
    ...
    node.clip=compositor.CreateContainerVisual();
    node.clip.Size({float(patch.width),float(patch.height)});
    node.clip.Offset({float(patch.source_x),float(patch.source_y),0.0f});
    node.clip.Clip(compositor.CreateInsetClip());
    ...
    if(scene.backdrop && !hidden_backdrop) {
    ...
    node.clip.Children().InsertAtTop(pixels);
...
inline SharedSparseScene make_shared_sparse_scene(
  ...
  scene.compositor=compositor;scene.backdrop=backdrop;
  ...
  update_shared_sparse_visuals(scene,surface,patches,opaque);
  return scene;
}
