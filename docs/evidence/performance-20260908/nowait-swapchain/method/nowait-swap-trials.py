...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreNowaitSwap';state=Path('/tmp/viewflow-nowait-swap-state.json')
def ps(code):
...
Path('/tmp/viewflow-restore-nowait-swap.ps1').write_text(restore)
...
try:
 ...
 paths=['platform/windows-composition-preview/main.cpp','platform/windows-composition-preview/stable_surface_layout.h','platform/windows-composition-preview/sparse_shared_visuals.h','platform/windows-composition-preview/sparse_visual_reference.h','target/release/vf-media-peer','crates/viewflowd/src/atlas_socket_trace.rs','platform/nvenc-encoder/gpu_dmabuf_encoder.cu','platform/windows-video-compositor/video_compositor.cpp','tools/windows_frame_observer.cpp','tools/windows_window_frame_observer.cpp','tools/windows_observer_timer.h'];Path('/tmp/viewflow-nowait-swap-trial-source-sha256.json').write_text(json.dumps({p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in paths},indent=2)+'\n')
 for label,codec,value in [('4k-nowait0-a1','h264','0'),('4k-nowait1-b1','h264','1'),('4k-nowait1-b2','h264','1'),('4k-nowait0-a2','h264','0')]:
  send=json.loads(send_path.read_text());receive=json.loads(original_receive);send['media']['color_codec']=codec;receive['color_codec']=codec;receive['native_presenter']=root+'\\native-nowait-build\\Release\\viewflow_windows_composition_preview.exe'
  ...
  present=[line for line in receiver.splitlines() if line.startswith('atlas-nowait-present ')]
  if value=='1':
   assert len(present)>20 and any('bound=1' in x and 'status=0' in x for x in present)
  else:assert not present
finally:
 try:
