...
static int coalesce_pixels() try {
  ...
  unsigned phase=0,bound_updates=0,swap_updates=0,busy_fallbacks=0;
  ...
  std::printf("bound_updates=%u swap_updates=%u busy_fallbacks=%u phases=%u\n",bound_updates,swap_updates,busy_fallbacks,phase);
  if(bound_updates<42 || swap_updates<70)throw std::runtime_error("bound updates were not exercised");
