...
int wmain(int argc,wchar_t**) try {
  ...
  auto blurred=host_effect(raw),candidate_blurred=host_effect(candidate_raw);
  ...
  for(unsigned phase=0;phase<4;++phase) {
    ...
    for(size_t i=0;i<bytes.size();i+=4){bytes[i]=bytes[i+1]=bytes[i+2]=0;bytes[i+3]=128;}
    const std::array<uint8_t,3> marker{uint8_t(80+phase*20),30,uint8_t(170-phase*20)};
    ...
    reference_commit_sparse_visuals(reference,reference_stage_sparse_visuals(reference,compositor,surface.surface,patches,0,256,256,blurred),256,256);
    ...
    while(std::chrono::steady_clock::now()<until) {
      ...
      bool fresh=true;for(unsigned panel=0;panel<3;++panel)for(unsigned c=0;c<3;++c)if(abs(sample(panel,8,8,c)-marker[c])>2)fresh=false;
      const std::array<int,3> expected_control=phase==0?std::array<int,3>{100,45,20}:
          (phase==1?std::array<int,3>{20,35,100}:std::array<int,3>{15,90,115});
      for(unsigned c=0;c<3;++c)if(abs(sample(2,64,64,c)-expected_control[c])>3)fresh=false;
  ...
  std::printf("host-swap bound_updates=%u swap_updates=%u\n",bound_updates,swap_updates);
  host_require(calibrate || (bound_updates>=2 && swap_updates==4),"swap path not exercised");
  host_require(blurred_difference>1000,"blurred host result did not differ from unblurred control");
  std::puts("PASS physical HostBackdrop reference/candidate equality, changing owned background, repacking; no input injection");
  return 0;
} catch(winrt::hresult_error const& e) {std::fprintf(stderr,"host-test HRESULT=%08x\n",unsigned(e.code()));return 1;}
  catch(std::exception const& e) {std::fprintf(stderr,"host-test error=%s\n",e.what());return 1;}

// GUI subsystem: the scheduled probe never needs a console window.
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR,int) {
  FILE* log=nullptr;
  if(_wfreopen_s(&log,L"host-backdrop-result.log",L"w",stdout)!=0)return 2;
