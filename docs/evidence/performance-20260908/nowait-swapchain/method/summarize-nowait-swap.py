...
for label in sys.argv[1:]:
 ...
 negative_commit=[x for x in pairs if x['commit_to_seen_ms']<0]
 assert bounds['negative_upper_bound']==0
 ...
 attempts_raw=t.rows(texts['receiver'],'atlas-nowait-present ')
 ...
 row['negative_commit_to_desktop_pairs']=negative_commit
 result.append(row)
...
Path('/tmp/viewflow-nowait-swap-comparison.json').write_text(json.dumps(result,indent=2)+'\n')
