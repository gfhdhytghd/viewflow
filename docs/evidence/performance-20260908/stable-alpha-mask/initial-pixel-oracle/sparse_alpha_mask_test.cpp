...
int wmain(int argc,wchar_t**) try {
  ...
  auto blurred=host_effect(raw),candidate_blurred=host_effect(candidate_raw),control_blurred=host_effect(compositor.CreateHostBackdropBrush());
  ...
  for(unsigned phase=0;phase<12;++phase) {
    ...
    reference_commit_sparse_visuals(reference,reference_stage_sparse_visuals(reference,compositor,surface.surface,patches,0,256,256,blurred),target_size,target_size);
    ...
    if(!retained_mask || (alpha_key!=previous_alpha && !(calibrate && phase>=4))) {
    ...
    for(auto& [key,node]:candidate.shared_sparse->nodes)if(node.masked)
