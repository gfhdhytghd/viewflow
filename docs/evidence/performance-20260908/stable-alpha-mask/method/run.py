...
assert mode in ['mask-toggle','mask-toggle-reverse'] and all(c.isalnum() or c=='-' for c in label)
source=r'''$r='__ROOT__';$label='__LABEL__';$mode='__MODE__';$probe='ViewflowPerf-MaskUpdateProbe';$observer='ViewflowPerf-MaskUpdateProbeObserver';$exe=$r+'\viewflow_windows_mask_update_probe.exe';$dd=$r+'\native-build\Release\viewflow_windows_frame_observer.exe';$pidSeen=0;$registeredProbe=$false;$registeredObserver=$false;$probeResult=-1;$observerResult=-1;
