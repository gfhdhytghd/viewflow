...
try {
 ...
 if('positive' -ne 'positive'){$a=New-ScheduledTaskAction -Execute $exe -WorkingDirectory $r -Argument 'positive'};
 ...
 $result=$i.LastTaskResult;Copy-Item ($r+'\alpha-mask-result.log') ($r+'\alpha-mask-positive.log');if($result -ne 0){throw ('unexpected oracle result '+$result)};
} finally {
 ...
 @{mode='positive';result=$result;expected=0;remaining=@(Get-CimInstance Win32_Process|Where-Object ExecutablePath -eq $exe).Count;tasks=@(Get-ScheduledTask -TaskName $task -ErrorAction SilentlyContinue).Count;exe=(Get-FileHash $exe|Select-Object Path,Hash);time=(Get-Date).ToString('o')}|ConvertTo-Json -Depth 4|Set-Content -Encoding UTF8 ($r+'\alpha-mask-positive-state.json');
}
Get-Content ($r+'\alpha-mask-positive-state.json');
