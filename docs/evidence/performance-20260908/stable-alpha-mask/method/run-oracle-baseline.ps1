...
try {
 ...
 if('baseline' -ne 'positive'){$a=New-ScheduledTaskAction -Execute $exe -WorkingDirectory $r -Argument 'baseline'};
 ...
 $result=$i.LastTaskResult;Copy-Item ($r+'\alpha-mask-result.log') ($r+'\alpha-mask-baseline.log');if($result -ne 1){throw ('unexpected oracle result '+$result)};
} finally {
 ...
 @{mode='baseline';result=$result;expected=1;remaining=@(Get-CimInstance Win32_Process|Where-Object ExecutablePath -eq $exe).Count;tasks=@(Get-ScheduledTask -TaskName $task -ErrorAction SilentlyContinue).Count;exe=(Get-FileHash $exe|Select-Object Path,Hash);time=(Get-Date).ToString('o')}|ConvertTo-Json -Depth 4|Set-Content -Encoding UTF8 ($r+'\alpha-mask-baseline-state.json');
}
Get-Content ($r+'\alpha-mask-baseline-state.json');
