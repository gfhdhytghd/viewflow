...
root=Path('/tmp/viewflow-mask-update-probe');results=[]
for label in ['probe-mask-ab','probe-mask-ba']:
 ...
 for x in rows:
  ...
  assert x['backgrounds']==4 and x['host_flag']==1
  assert x['mask_reused']==int((x['phase']%2!=0)!=(header['mode']=='mask-toggle-reverse'))
 ...
 for phase in sorted({p['phase'] for p in pairs}):
  ...
  phases.append({'phase':phase,'mask_reused':selected[0]['mask_reused'],'first_frame':selected[0]['frame'],'last_frame':selected[-1]['frame'],**stat(selected)})
 # Aggregated latency is valid; do not aggregate change rate across noncontiguous phases.
 grouped={}
 for bg in [0,1]:
  grouped[str(bg)]=stat([p for p in pairs if p['mask_reused']==bg and not p['warmup']]);del grouped[str(bg)]['observed_change_hz']
...
(root/'mask-comparison.json').write_text(json.dumps(results,indent=2)+'\n')
