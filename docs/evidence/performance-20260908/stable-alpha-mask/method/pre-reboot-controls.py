...
root=r'C:\Users\wilf\Viewflow\perf-isolated-8e97c1b751fc';task='ViewflowPerf-RestoreMaskUpdateProbe';state=Path('/tmp/viewflow-mask-update-state.json')
def ps(code):
...
Path('/tmp/viewflow-restore-mask-update.ps1').write_text(restore)
...
try:
 ...
 for label,mode in [('probe-mask-ab','mask-toggle'),('probe-mask-ba','mask-toggle-reverse')]:
  ...
  with open('/tmp/viewflow-mask-update-probe/'+label+'-control.log','w') as log:
   r=subprocess.run(['python3','docs/evidence/performance-20260908/stable-alpha-mask/method/run.py',label,mode],stdout=log,stderr=subprocess.STDOUT)
