// Full-glass mask invalidation probe. Owns one nonactivating 4K-content window.
// Alpha is constant; compare per-frame color surface vs retained alpha source.
// Both modes retain all four backgrounds and live HostBackdrop sigma=12.
...
struct Timing {
  uint32_t frame{};
  uint32_t phase{}, phase_frame{}, backgrounds{},host_flag{},mask_reused{};
...
float4 pixel(float4 position:SV_Position):SV_Target {
 ...
 float alpha=(p.x<124 || p.x>=3716 || p.y<124 || p.y>=2276)?0.5:1;
 if(p.x>=32&&p.x<2080&&p.y>=32&&p.y<96)alpha=1;
...
struct Wuc {
  ...
  bool reverse{};
  CompositionDrawingSurface alpha_surface{nullptr};
  CompositionSurfaceBrush alpha_brush{nullptr};
  HWND window{};
  Wuc(HWND window,Renderer& renderer,const std::wstring& mode){
    ...
    reverse=mode==L"mask-toggle-reverse";
    alpha_surface=graphics.CreateDrawingSurface({float(width),float(height)},winrt::Windows::Graphics::DirectX::DirectXPixelFormat::B8G8R8A8UIntNormalized,winrt::Windows::Graphics::DirectX::DirectXAlphaMode::Premultiplied);
    alpha_brush=compositor.CreateSurfaceBrush(alpha_surface);
    alpha_brush.Stretch(CompositionStretch::None);
    alpha_brush.HorizontalAlignmentRatio(0);alpha_brush.VerticalAlignmentRatio(0);
    {
  ...
  void submit(Renderer& renderer,Timing& t){
    ...
    t.mutation=qpc();
    if(t.frame==1) {
      auto alpha_drawing=alpha_surface.as<ABI::Windows::UI::Composition::ICompositionDrawingSurfaceInterop>();
      com_ptr<ID3D11Texture2D> alpha_target;POINT alpha_offset{};
      check_hresult(alpha_drawing->BeginDraw(nullptr,__uuidof(ID3D11Texture2D),alpha_target.put_void(),&alpha_offset));
      try{renderer.copy(alpha_target.get(),alpha_offset);}catch(...){alpha_drawing->EndDraw();throw;}
      check_hresult(alpha_drawing->EndDraw());renderer.context->Flush();
    }
    t.phase=(t.frame-1)/240;t.phase_frame=(t.frame-1)%240;
    const bool separate=(t.phase%2!=0)!=reverse;
    t.backgrounds=4;t.host_flag=1;t.mask_reused=unsigned(separate);
    if(t.phase_frame==0) {
      for(auto& [key,node]:scene->nodes)if(node.masked)
        node.masked.SetSourceParameter(L"atlas",separate?alpha_brush:scene->brush);
    }
...
int run(const std::wstring& mode,UINT duration,FILE* log){
  ...
  Window window;
  window.value=CreateWindowExW(WS_EX_NOREDIRECTIONBITMAP|WS_EX_NOACTIVATE|WS_EX_TOOLWINDOW,cls.lpszClassName,L"Viewflow owned full-glass mask probe",WS_POPUP,monitor.rect.left-4,monitor.rect.top-4,width,height,nullptr,nullptr,cls.hInstance,nullptr);
  if(!window.value)throw_last_error();
  BOOL enabled=TRUE;check_hresult(DwmSetWindowAttribute(window.value,DWMWA_USE_HOSTBACKDROPBRUSH,&enabled,sizeof(enabled)));
  std::fprintf(log,"probe-scene host_flag=1 sparse=1 backgrounds=4 sigma=12 full_glass=1 mask_alpha_static=1 boundary_alpha=128 center_alpha=255\n");
  std::fprintf(log,"probe-toggle phase_frames=240 warmup_frames=30 nodes_retained=1 reverse=%u\n",unsigned(mode==L"mask-toggle-reverse"));
  ...
  for(const auto& t:timings)std::fprintf(log,"probe-frame frame=%u phase=%u phase_frame=%u backgrounds=%u host_flag=%u mask_reused=%u render_qpc=%lld drawn_qpc=%lld begin_qpc=%lld copied_qpc=%lld ended_qpc=%lld mutation_qpc=%lld committed_qpc=%lld\n",t.frame,t.phase,t.phase_frame,t.backgrounds,t.host_flag,t.mask_reused,t.render,t.drawn,t.begin,t.copied,t.ended,t.mutation,t.committed);
...
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR,int){
  ...
  const bool valid=(mode==L"mask-toggle"||mode==L"mask-toggle-reverse")&&end&&!*end&&duration>=1000&&duration<=60000;LocalFree(args);if(!valid)return 2;
