...
labels=sys.argv[1:] or ['4k-pacing-off-a1','4k-pacing-eight-b1','4k-pacing-eight-b2','4k-pacing-off-a2']
for label in labels:
 ...
 assert all(x['datagrams']==1 for x in sends)
 groups=[];batch=[]
 for x in sends:
  if batch and x['before_ns']-batch[-1]['before_ns']>1e6:
   groups.append({'datagrams':len(batch),'span_ms':(batch[-1]['after_ns']-batch[0]['before_ns'])/1e6,'begin_ns':batch[0]['before_ns']});batch=[]
  batch.append(x)
 if batch:groups.append({'datagrams':len(batch),'span_ms':(batch[-1]['after_ns']-batch[0]['before_ns'])/1e6,'begin_ns':batch[0]['before_ns']})
 ...
 row=dict(label=label,pacing=t.rows(s,'atlas-udp-pacing '),native_commit_hz=(len(native)-1)*native[0]['frequency']/(native[-1]['qpc']-native[0]['qpc']),feedback_ms=t.stats(vals),feedback_over_33ms=sum(x>33.333333 for x in vals),feedback_over_100ms=sum(x>100 for x in vals),feedback_excess_over_33ms_sum=sum(max(x-33.333333,0) for x in vals),wall_stage_ms=stages['wall_stage_ms'],trace_counts=traceends,source_send_group_sizes=t.stats([x['datagrams'] for x in groups]),source_send_group_gap_ms=t.stats([(b['begin_ns']-a['begin_ns'])/1e6 for a,b in zip(groups,groups[1:])]),source_send_count=len(sends),slow_frames=slow)
 results.append(row);Path('/tmp/'+label+'-pacing-stages.json').write_text(json.dumps(stages,indent=2)+'\n')
...
Path('/tmp/viewflow-udp-pacing-comparison.json').write_text(json.dumps(results,indent=2)+'\n')
