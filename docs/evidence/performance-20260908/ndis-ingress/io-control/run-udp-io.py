...
serverlog=Path('/tmp/viewflow-udp-burst-'+label+'-receiver.log');script="$env:VIEWFLOW_PROBE_IO='"+os.environ.get('VIEWFLOW_PROBE_IO','blocking')+"'; & '"+root+"\\udp-io-probe.exe' '"+format(nonce,'x')+"' "+str(port)+" "+str(len(counts))+"; exit $LASTEXITCODE"
