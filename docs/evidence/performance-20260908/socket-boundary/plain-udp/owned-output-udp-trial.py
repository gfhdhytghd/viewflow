...
state=Path('/tmp/viewflow-owned-output-udp-state.json');data={};output=None
...
try:
 ...
 output=new[0]['name'];assert re.fullmatch(r'HEADLESS-\d+',output)
 ...
 subprocess.run(['python3','/tmp/viewflow-active-udp-trial.py'],env=env,check=True)
finally:
