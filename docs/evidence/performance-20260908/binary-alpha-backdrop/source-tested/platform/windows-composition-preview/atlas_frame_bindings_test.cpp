...
int main() {
  ...
  for(auto id:{1,2,3,4,5}) {
    ...
    if(b->binary_tiles!=std::vector<uint8_t>{uint8_t(id<=4)})return 32;
  }
  // Same immutable alpha can classify differently after atlas remapping; each
  // queued decoder identity must retain its own result when later alpha changes.
  AtlasFrameBindings binary;
  auto binary1=frame(1),binary2=frame(2),fractional=frame(3),remapped=frame(4);
  auto samples=std::vector<uint8_t>(64,255);samples[7]=128;samples[2*8+2]=0;
  const auto plane=std::make_shared<const std::vector<uint8_t>>(samples);
  for(auto* f:{&binary1,&binary2,&fractional,&remapped})
    f->atlas->patches=std::vector<viewflow::vfgp::AtlasPatch>{{0,0,0,0,0,4,4}};
  binary1.shared_alpha=plane;binary2.shared_alpha=plane;remapped.shared_alpha=plane;
  samples[3*8+3]=254;fractional.shared_alpha=std::make_shared<const std::vector<uint8_t>>(samples);
  remapped.atlas->patches->at(0).x=4;remapped.atlas->revision=2;
  if(!binary.Stage(binary1)||!binary.Stage(binary2)||!binary.Stage(fractional)||!binary.Stage(remapped))return 33;
  for(auto id:{1,2,3,4}) {
    const auto* b=binary.Find(id,8,8);
    if(!b||b->binary_tiles!=std::vector<uint8_t>{uint8_t(id<=2)})return 34;
  }
  return 0;
}
