...
inline bool AllBinaryAlpha(std::span<const uint8_t> row) {
  size_t offset=0;
  for (; offset+sizeof(uint64_t)<=row.size(); offset+=sizeof(uint64_t)) {
    uint64_t word;
    std::memcpy(&word,row.data()+offset,sizeof(word));
    // Adjacent bits within each byte must agree; ignore byte boundaries.
    if ((word ^ (word >> 1)) & UINT64_C(0x7f7f7f7f7f7f7f7f)) return false;
  }
  for (; offset<row.size(); ++offset)
    if (row[offset]!=0 && row[offset]!=255) return false;
  return true;
}

// At integer 1:1 sampling, binary alpha contributes no masked backdrop beneath
// foreground pixels: A*(1-A)==0. This classification alone does not permit
// elision when the presenter scales or fractionally offsets the visual.
inline std::vector<uint8_t> BinarySparseTiles(std::span<const uint8_t> alpha,
    uint32_t width,uint32_t height,std::span<const vfgp::AtlasPatch> patches,
    size_t tile_count) {
  std::vector<uint8_t> binary(tile_count),seen(tile_count);
  if (!width || !height || uint64_t(width)*height!=alpha.size()) return binary;
  std::fill(binary.begin(),binary.end(),uint8_t(1));
  for (const auto& patch:patches) {
    if (patch.tile_index>=tile_count || !patch.width || !patch.height ||
        uint64_t(patch.x)+patch.width>width || uint64_t(patch.y)+patch.height>height)
      return std::vector<uint8_t>(tile_count);
    seen[patch.tile_index]=1;
    if (!binary[patch.tile_index]) continue;
    for (uint32_t y=patch.y;y<patch.y+patch.height;++y) {
      if (!AllBinaryAlpha(alpha.subspan(size_t(y)*width+patch.x,patch.width))) {
        binary[patch.tile_index]=0;break;
      }
    }
  }
  for (size_t i=0;i<tile_count;++i) binary[i]&=seen[i];
  return binary;
}
