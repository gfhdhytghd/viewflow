...
struct AtlasFrameBinding {
  ...
  std::vector<uint8_t> binary_tiles;
};
  ...
  bool Stage(const vfgp::Frame &frame) {
    ...
    if (layout.patches) {
      if (frame.shared_alpha && frame.shared_alpha == opaque_alpha_ && latest_ &&
          ...
          layout.tiles.size() == latest_->layout.tiles.size()) {
        binding.opaque_patches = latest_->opaque_patches;
        binding.binary_tiles = latest_->binary_tiles;
      } else {
        binding.opaque_patches = OpaqueSparsePatches(frame.Alpha(), frame.width, frame.height, *layout.patches);
        binding.binary_tiles = BinarySparseTiles(frame.Alpha(), frame.width, frame.height,
            *layout.patches,layout.tiles.size());
      }
    }
