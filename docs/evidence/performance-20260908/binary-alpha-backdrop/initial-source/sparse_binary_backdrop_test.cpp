...
int wmain(int argc,wchar_t**) try {
  ...
  for(const auto& test:cases) {
    ...
    const std::array<uint8_t,3> marker{uint8_t(80+phase*11),30,uint8_t(170-phase*11)};
    ...
    // Reference always keeps the production effect. Candidate tests eligibility;
    // the forced-muted control must expose incorrect elision on negative cases.
    for(auto* fg:{&reference,&candidate,&control}) {
      auto backdrop=fg==&reference?raw:(fg==&candidate?candidate_raw:control_raw);
      commit_sparse_visuals(*fg,stage_sparse_visuals(*fg,compositor,surface.surface,patches,0,256,256,backdrop,12),256,256);
    }
    set_backdrops(reference,false);set_backdrops(candidate,test.mute);set_backdrops(control,true);
    for(auto* fg:{&reference,&candidate,&control}){
      fg->sparse_root.Scale({test.scale,test.scale,1});fg->visual.Offset({test.offset,test.offset,0});
    }
  ...
  std::puts("PASS binary alpha identity elision and fractional alpha/transform negative controls; no input injection");
  return 0;
