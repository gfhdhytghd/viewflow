...
for label in sys.argv[1:]:
 ...
 row['diagnostic_mode']='minimal'
 row['binary_nodes']=[t.numbers(x) for x in t.rows(texts['receiver'],'atlas-sparse-nodes ')]
 disabled='-disabled-' in label
 assert row['binary_nodes'] and all(x['binary_alpha']==1 and x['backdrop_muted']==int(not disabled) for x in row['binary_nodes'])
...
Path('/tmp/viewflow-binary-backdrop-comparison.json').write_text(json.dumps(result,indent=2)+'\n')
