...
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR,int) {
  ...
  if(!argv || argc!=5)return 2;
  std::wstring executable=argv[1],config=argv[2],directory=argv[3],disabled=argv[4];LocalFree(argv);
  ...
  SetEnvironmentVariableW(L"VIEWFLOW_QUIC_SOCKET_TRACE", L"0");
  SetEnvironmentVariableW(L"VIEWFLOW_ATLAS_GPU_QUERIES",L"0");
  SetEnvironmentVariableW(L"VIEWFLOW_ATLAS_DIAGNOSTIC_NO_BINARY_BACKDROP",disabled.c_str());
