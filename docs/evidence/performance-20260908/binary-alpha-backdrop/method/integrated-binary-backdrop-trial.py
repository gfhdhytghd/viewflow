...
subprocess.run(['python3','/tmp/viewflow-start-binary-backdrop-receiver.py'],check=True)
observer=subprocess.Popen(['python3',('/tmp/viewflow-observe-both.py' if os.environ.get('VIEWFLOW_OBSERVE_BOTH')=='1' else '/tmp/viewflow-observe-binary-backdrop.py'),label]) if os.environ.get('VIEWFLOW_OBSERVE_FRAMES')=='1' else None
