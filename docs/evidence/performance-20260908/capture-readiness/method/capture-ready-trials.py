...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreCaptureReady';state=Path('/tmp/viewflow-capture-ready-state.json')
def ps(code):
...
Path('/tmp/viewflow-restore-capture-ready.ps1').write_text(restore)
...
try:
 ...
 paths=['crates/viewflowd/Cargo.toml','crates/viewflowd/src/atlas_source.rs','crates/viewflowd/src/gpu_atlas_capture.rs','crates/viewflowd/src/gpu_atlas_device.rs','crates/viewflowd/src/gpu_atlas_session.rs','crates/viewflowd/src/hyprcapture_gpu_socket.rs','platform/windows-composition-preview/main.cpp','platform/windows-video-compositor/gpu_timestamp_probe.h','target/release/vf-media-peer','tools/windows_frame_observer.cpp','tools/windows_observer_timer.h'];Path('/tmp/viewflow-capture-ready-trial-source-sha256.json').write_text(json.dumps({p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in paths},indent=2)+'\n')
 for label,codec,value,capture_events in [('4k-capture-ready0-minimal-a1','h264','minimal','0'),('4k-capture-ready1-minimal-b1','h264','minimal','1'),('4k-capture-ready1-minimal-b2','h264','minimal','1'),('4k-capture-ready0-minimal-a2','h264','minimal','0')]:
