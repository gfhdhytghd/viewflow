...
import json
out=Path('docs/evidence/performance-20260908/capture-readiness');rs=json.loads((out/'comparison.json').read_text())
s='''# 捕获帧到达唤醒：减少源端轮询等待
...
'''
for r in rs:
 c=r['capture_delivery_ms']['socket_age_max_us'];u=r['bounds']['unique_capture_stage_samples']['capture_to_desktop_upper_ms'];s+=f"| {r['label']} | {c['median']:.3f} | {c['p95']:.3f} | {r['native_commit_hz']:.3f} | {r['desktop']['observed_changes_per_second']:.3f} | {u['median']:.3f} | {u['p95']:.3f} | {u['max']:.3f} |\n"
s+='''
...
'''
for r in rs:
 b=r['bounds'];u=b['unique_capture_stage_samples'];s+=f"| {r['label']} | {u['capture_to_mutation_upper_ms']['median']:.3f} | {u['mutation_to_desktop_ms']['median']:.3f} | {u['render_to_capture_ms']['median']:.3f} | {b['latency_upper_ms']['median']:.3f} |\n"
s+='''
...
'''
for r in rs:
 t=r['capture_retire'][0];s+=f"| {r['label']} | {t['waiting']} | {t['capture_wakes']} | {t['periodic_wakes']} | {t['expired_clean']} | {t['enqueued']} |\n"
s+='''
