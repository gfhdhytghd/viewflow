...
async fn run_live(
    ...
    let capture_events = std::env::var("VIEWFLOW_CAPTURE_EVENTS").as_deref() != Ok("0");
    let mut capture_wakes = 0_u64;
    let mut periodic_wakes = 0_u64;
    eprintln!("atlas-capture-wait events_enabled={capture_events} periodic_fallback_ms=1");
    let result = async {
        ...
        loop {
            ...
            match poll {
                AtlasDevicePoll::Waiting => {
                    // Keep the existing input/enrollment/feedback poll cadence
                    // as a fallback, but wake immediately for an arriving frame.
                    let arrived = if capture_events {
                        tokio::select! {
                            () = session.capture_readable() => true,
                    ...
                    if arrived {
                        capture_wakes = capture_wakes.saturating_add(1);
                    } else {
                        periodic_wakes = periodic_wakes.saturating_add(1);
                    }
                },
    ...
    .await;
    eprintln!(
        "atlas-source-retiring waiting={waiting} expired_clean={expired} enqueued={enqueued} capture_wakes={capture_wakes} periodic_wakes={periodic_wakes} physical_present_receipt=false"
    );
