...
state={'commit_mode':os.environ['VIEWFLOW_COMMIT_MODE']};source=fixture=None;output=None;tasks=[];rule=False
...
try:
 ...
 nested=json.loads(Path(os.environ['VIEWFLOW_GLASS_NESTED_STATE']).read_text());screen=nested['screen']
 m=next(m for m in info('monitors','all') if m['name']==screen);assert (m['width'],m['height'],m['scale'])==(3840,2400,2)
 lua('_G.viewflow_full_glass_fixture_rule=hl.window_rule({name="viewflow-full-glass-fixture-owned",match={class="^viewflow-full-glass-fixture$"},monitor='+json.dumps(screen)+',workspace='+json.dumps(str(m['activeWorkspace']['id'])+' silent')+',float=true,size="1920 1200",move="0 0",no_initial_focus=true,no_focus=true,no_anim=true,no_shadow=true,no_blur=true,border_size=0,rounding=0})');rule=True
 env=os.environ.copy();env['QT_QPA_PLATFORM']='wayland'
 state['before_fixture_focus']=info('activewindow')
 with (out/'fixture.log').open('w') as log:fixture=subprocess.Popen([str(local/'fixture-build/viewflow_linux_frame_fixture'),screen,'150000'],env=env,stdout=log,stderr=subprocess.STDOUT)
 for _ in range(100):
