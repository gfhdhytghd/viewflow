...
root=pathlib.Path(tempfile.mkdtemp(prefix='viewflow-phase-follow-nested.')); runtime=pathlib.Path(tempfile.mkdtemp(prefix='vfc.'))
...
def save():
 (root/'state.json').write_text(json.dumps(state,indent=2)+'\n');pathlib.Path('/tmp/viewflow-phase-follow-nested-latest.json').write_text(json.dumps(state,indent=2)+'\n')
def alive(p):
...
try:
 ...
 config.write_text('''hl.monitor({output="",mode="3840x2400@60",position="auto",scale=2})
...
hl.window_rule({name="owned-fixture",match={class="^viewflow-full-glass-fixture$"},float=true,size="1920 1200",move="0 0",no_initial_focus=true,no_focus=true,no_anim=true,border_size=0,rounding=0})
''')
 ...
 ctl('eval','hl.monitor({output='+json.dumps(output)+',mode="3840x2400@60",position="6000x0",scale=1})')
 for _ in range(40):
  m=[x for x in info('monitors') if x['name']==output]
  if m and m[0]['activeWorkspace']['id']>0 and m[0]['width']==3840 and m[0]['height']==2400 and m[0]['scale']==1:break
  time.sleep(.1)
 else:raise RuntimeError('outer output unavailable')
 ws=m[0]['activeWorkspace']['id'];state['outer_monitor']=m[0]
 state['focus_after_output']=info('activewindow')
 ...
 ctl('eval','hl.exec_cmd('+json.dumps(str(launcher))+', {no_initial_focus=true,no_focus=true,monitor='+json.dumps(output)+',workspace='+json.dumps(str(ws)+' silent')+',float=true,size="3840 2400",move="0 0",no_anim=true,no_shadow=true,no_blur=true})')
 for _ in range(120):
 ...
 assert info('activewindow').get('pid')!=pid,'nested outer window accepted focus'
 ...
 innermon=[x for x in info('monitors','all',env=inner) if x['name'].startswith('WAYLAND-')];assert len(innermon)==1
 ...
 ctl('eval','hl.window_rule({name="owned-fixture-screen",match={class="^viewflow-full-glass-fixture$"},monitor='+json.dumps(screen)+',workspace='+json.dumps(str(settled[0]['activeWorkspace']['id'])+' silent')+',no_initial_focus=true})',env=inner)
 so='/tmp/viewflow-phase-follow-build/viewflow-capture-phase-test.so'
 ...
 assert 'function' in ctl('repl','return type(hl.plugin.viewflow_capture_phase_test.window_stream_start_commit)',env=inner)
 save();print('NESTED_READY',root,signature,pid,screen,flush=True)
 inner['VIEWFLOW_COMMIT_STATE']=str(root/'state.json')
 subprocess.run(['/usr/bin/python3','/home/wilf/data/viewflow/docs/evidence/performance-20260908/capture-phase-follow/method/cohort-smoke.py'],env=inner,check=True)
 state['smoke']='passed';save()
 inner['VIEWFLOW_GLASS_NESTED_STATE']=str(root/'state.json')
 inner['PATH']='/tmp/viewflow-phase-follow-shim:'+inner['PATH']
 for label,mode in [('phase-grid-a1','grid'),('phase-follow-b1','commit'),('phase-follow-b2','commit'),('phase-grid-a2','grid')]:
  inner['VIEWFLOW_COMMIT_MODE']=mode
  print('TRIAL_START',label,flush=True)
  subprocess.run(['/usr/bin/python3','/home/wilf/data/viewflow/docs/evidence/performance-20260908/capture-phase-follow/method/run-nested-glass.py',label],env=inner,check=True,timeout=180)
 ...
 if output:
  monitor=next((m for m in info('monitors','all') if m['name']==output),None)
  remaining=[x for x in info('clients') if monitor and x['monitor']==monitor['id']]
 ...
 state['focus_changed']=state['after_focus'].get('address')!=state['before_focus'].get('address');save()
