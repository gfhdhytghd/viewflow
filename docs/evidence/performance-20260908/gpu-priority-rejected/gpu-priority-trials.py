...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreGpuPriorityControl';state=Path('/tmp/viewflow-gpu-priority-state.json')
def ps(code):
...
Path('/tmp/viewflow-restore-gpu-priority-control.ps1').write_text(restore)
...
try:
 ...
 paths=['target/release/vf-media-peer','crates/viewflowd/src/atlas_socket_trace.rs','platform/nvenc-encoder/gpu_dmabuf_encoder.cu','platform/windows-video-compositor/video_compositor.cpp','tools/windows_frame_observer.cpp'];Path('/tmp/viewflow-gpu-priority-trial-source-sha256.json').write_text(json.dumps({p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in paths},indent=2)+'\n')
 for label,value in [('4k-gpu-priority-0-a1',0),('4k-gpu-priority-7-b1',7),('4k-gpu-priority-7-b2',7),('4k-gpu-priority-0-a2',0)]:
  ...
  phase={'label':label,'requested_gpu_relative_priority':value,'before':before};data['phases'].append(phase);state.write_text(json.dumps(data,indent=2)+'\n');print('BEGIN',label,before,flush=True)
  env=dict(os.environ,VIEWFLOW_OBSERVE_FRAMES='1',VIEWFLOW_OBSERVE_PHYSICAL4K='1',VIEWFLOW_OBSERVER_EXE='viewflow_windows_frame_observer.exe',VIEWFLOW_ALPHA_COPY_PROFILE='1',VIEWFLOW_ALPHA_OUTPUT_COPY='0',VIEWFLOW_TRIAL_GPU_TIMINGS='all',VIEWFLOW_GPU_TILE_COPY='0',VIEWFLOW_QUIC_POLL='0',VIEWFLOW_QUIC_SOCKET_TRACE='1',VIEWFLOW_TRIAL_GPU_PRIORITY=str(value));env.pop('VIEWFLOW_QUIC_BURST_PACKETS',None)
  ...
  receiver=Path('/tmp/viewflow-integrated-pair/receiver-'+label+'.log').read_text();lines=[x for x in receiver.splitlines() if x.startswith('gpu-relative-priority ')];assert len(lines)==1,lines
  assert f'requested={value} before=0 observed={value} query_status=0 before_status=0 apply_status=0 after_status=0' in lines[0],lines
finally:
 if registered:
  ps(restore)
