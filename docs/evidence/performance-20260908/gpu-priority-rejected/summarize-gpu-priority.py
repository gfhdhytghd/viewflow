...
for label in sys.argv[1:]:
 ...
 row['gpu_relative_priority']=[t.numbers(r) for r in t.rows(texts['receiver'],'gpu-relative-priority ')]
 assert len(row['gpu_relative_priority'])==1
 result.append(row)
...
Path('/tmp/viewflow-gpu-priority-comparison.json').write_text(json.dumps(result,indent=2)+'\n')
