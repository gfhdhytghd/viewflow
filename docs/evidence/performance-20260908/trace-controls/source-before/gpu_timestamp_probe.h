...
class GpuTimestampScope {
 public:
  ...
  GpuTimestampScope(ID3D11Device* device,ID3D11DeviceContext* context,
    ...
    static const bool enabled=[] {wchar_t v[4]{};return GetEnvironmentVariableW(L"VIEWFLOW_ATLAS_TIMINGS",v,4)==3 && wcscmp(v,L"all")==0;}();
    if(!enabled || frame<4 || frame%20!=sample_remainder || !device || !context) return;
    try {
