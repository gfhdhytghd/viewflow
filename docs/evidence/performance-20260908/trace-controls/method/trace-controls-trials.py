...
root=Path('/tmp/viewflow-windows-isolated-root.txt').read_text().strip();task='ViewflowPerf-RestoreTraceControls';state=Path('/tmp/viewflow-trace-controls-state.json')
def ps(code):
...
Path('/tmp/viewflow-restore-trace-controls.ps1').write_text(restore)
...
try:
 ...
 paths=['platform/windows-composition-preview/main.cpp','platform/windows-video-compositor/gpu_timestamp_probe.h','target/release/vf-media-peer','crates/viewflowd/src/atlas_socket_trace.rs','platform/nvenc-encoder/gpu_dmabuf_encoder.cu','platform/windows-video-compositor/video_compositor.cpp','tools/windows_frame_observer.cpp','tools/windows_window_frame_observer.cpp','tools/windows_observer_timer.h'];Path('/tmp/viewflow-trace-controls-trial-source-sha256.json').write_text(json.dumps({p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in paths},indent=2)+'\n')
 for label,codec,value in [('4k-trace-full-a1','h264','full'),('4k-trace-noquery-b1','h264','noquery'),('4k-trace-minimal-c1','h264','minimal'),('4k-trace-minimal-c2','h264','minimal'),('4k-trace-noquery-b2','h264','noquery'),('4k-trace-full-a2','h264','full')]:
  ...
  phase={'label':label,'codec':codec,'diagnostic_mode':value,'before':before};data['phases'].append(phase);state.write_text(json.dumps(data,indent=2)+'\n');print('BEGIN',label,before,flush=True)
