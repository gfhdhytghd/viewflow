...
if os.environ.get('VIEWFLOW_TRIAL_INPLACE') in ('0','1'):
 s=s.replace('isolated-receiver-runner-socket-trace.exe','isolated-receiver-runner-inplace-'+os.environ['VIEWFLOW_TRIAL_INPLACE']+'.exe')
if os.environ.get('VIEWFLOW_TRIAL_NOWAIT') in ('0','1'):
 s=s.replace('isolated-receiver-runner-socket-trace.exe','isolated-receiver-runner-nowait-'+os.environ['VIEWFLOW_TRIAL_NOWAIT']+'.exe')
if os.environ.get('VIEWFLOW_TRIAL_TRACE_MODE') in ('full','noquery','minimal'):
 name='isolated-receiver-runner-trace-'+os.environ['VIEWFLOW_TRIAL_TRACE_MODE']+'.exe'
 s=s.replace('isolated-receiver-runner-socket-trace.exe',name).replace('isolated-receiver-runner.exe',name)
