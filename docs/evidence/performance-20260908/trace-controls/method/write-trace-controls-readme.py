...
import json
out=Path('docs/evidence/performance-20260908/trace-controls');rows=json.loads((out/'comparison.json').read_text())
s='''# GPU 查询与日志开销：六轮对照
...
'''
for r in rows:
 u=r['bounds']['unique_capture_stage_samples'];c=u['capture_to_desktop_upper_ms'];s+=f"| {r['label']} | {r['native_commit_hz']:.3f} | {r['desktop']['observed_changes_per_second']:.3f} | {c['median']:.3f} | {c['p95']:.3f} | {c['max']:.3f} | {u['capture_to_mutation_upper_ms']['median']:.3f} | {u['mutation_to_desktop_ms']['median']:.3f} |\n"
s+='''
...
'''
for r in rows:
 w=r['clock_and_stages']['wall_stage_ms'];s+=f"| {r['label']} | {w['encode (host incl waits)']['median']:.3f} | {w['encoded to pipe admission (transport + receiver + pipe parse)']['median']:.3f} | {w['pipe admission to decoded']['median']:.3f} | {r['log_bytes']['source']/1e6:.2f} | {r['log_bytes']['receiver']/1e6:.2f} | {r['gpu_timestamp_count']} / {r['gpu_completion_count']} |\n"
s+='''
